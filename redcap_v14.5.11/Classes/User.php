...
class User
{
	...
	public static function getUsersDataResRespond($appendFirstLastName=true, $excludeUserID=null, $record=null)
	{
		...
		if (is_numeric($user_rights['group_id']) && $recordDag == null) {
            ...
            $sql .= " and (u.group_id is null or u.group_id = '$recordDag')";
        } elseif (is_numeric($user_rights['group_id']) && isinteger($recordDag)) {
            $sql .= " and (u.group_id is null or u.group_id = '{$user_rights['group_id']}' or u.group_id = '$recordDag')";
        }
	...
	public static function renderSponsorDashboard($controlCenterView=false, $displayOnlyTheseUiIds=array())
	{
		...
		if (!$controlCenterView && isset($_GET['requester']) && is_numeric($_GET['requester'])) {
					...
					switch ($_POST['type']) {
						...
						case 'resend account creation email':
							foreach ($selectedUsers as $thisUiId) {
								...
								$email->setFrom($GLOBALS['user_email']);
	...
	public static function renderTwoFactorInstructionsAuthenticator($username, $emailEmbed=false, $displayOpenInNewWindow=true, $displayQRcode=true)
	{
		...
		// Dialog content
		return	RCView::div(array('id'=>'two_factor_totp_setup', 'class'=>'simpleDialog', 'title'=>$lang['system_config_713'], 'style'=>'font-size:13px;'),
					// Instructions
    ...
    public static function notifyAdminNewUser($username)
    {
        ...
        if (!isEmail($email)) return false;
        // Send the email
        return REDCap::email($GLOBALS['project_contact_email'], $GLOBALS['project_contact_email'], "[REDCap] ".$lang['system_config_793']." \"$username\"",
    ...
    public static function notifyNewUserWelcomeEmail($username)
    {
        ...
        if (!isEmail($email)) return false;
        // Send the email
        return REDCap::email($email, $GLOBALS['project_contact_email'], "[REDCap] ".$lang['system_config_800']." \"$username\"",
