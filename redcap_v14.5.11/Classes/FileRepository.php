...
class FileRepository
{
    ...
    public static function createFolder()
    {
        ...
        $sql = "insert into redcap_docs_folders (project_id, name, parent_folder_id, dag_id, role_id) values 
                (".PROJECT_ID.", '".db_escape($_POST['name'])."', ".checkNull($_POST['folder_id']).", ".checkNull($dag_id).", ".checkNull($role_id).")";
        if (!db_query($sql)) {
    ...
    public static function deleteNow($doc_id)
    {
        ...
        Files::deleteFilePermanently($storedName);
        ...
        elseif ($user_rights['data_export_tool'] == '1' && isset($_GET['doc_id']) && $_GET['doc_id'] == 'pdf_archive_all')
        {
            ...
            if ($zip->open($target_zip, ZipArchive::CREATE) === TRUE)
            {
                foreach ($files as $file)
                {
                    ...
                    if ($user_rights['forms_export'][$Proj->surveys[$file['survey_id']]['form_name']] != '1') continue;
                    // Get file attr and content
    ...
    public static function getBreadcrumbs($folder_id, $type, $recycle_bin=0)
    {
        ...
        elseif ($type == 'pdf_archive') {
            $showDownloadBtn = ($user_rights['data_export_tool'] == '1');
            $downloadBtn = "";
            if ($showDownloadBtn) {
                $downloadBtn = RCView::div(['class'=>'float-end'], RCView::button(array('class'=>'btn btn-xs btn-defaultrc', 'style'=>'color:green;font-size:13px;margin-top:5px;', 'onclick'=>"window.location.href=app_path_webroot + 'index.php?pid=' + pid + '&route=FileRepositoryController:download&doc_id=pdf_archive_all';"), '<i class="fas fa-file-archive"></i> '.RCView::tt("docs_71")));
            }
            return "<div class='ItemListBreadcrumbSeparator'>/</div><div class='ItemListBreadcrumb'>".RCView::tt('econsent_115')."</div>$downloadBtn";
        }
        // Data Export files
        elseif ($type == 'export') {
    ...
    public static function getFileList($folder_id=null, $type=null, $recycle_bin=0)
	{
                ...
                if ($recycle_bin == 0)
                {
                    ...
                    foreach ($folders as $thisFolderId=>$theseFolderNames) {
                        ...
                        $rows[] = [0=>RCView::checkbox(['id'=>'folder-select-'.$thisFolderId, 'class'=>'folder-select opacity50']), 1=>"<i class='fa-solid fa-folder fs20 me-1 text-primaryrc'></i> <a id='file-folder-$thisFolderId' href='javascript:;' class='fs14' onclick=\"loadFileRepoTable('$thisFolderId','',0);\">".strip_tags($theseFolderNames)."</a>"
                            . "<a id='folder-rename-$thisFolderId' href='javascript:;' class='fs12 opacity65 ms-3 folder-rename invisible' title='" . RCView::tt_js('docs_102') . "' onclick=\"fileRepoFolderRename($thisFolderId);\"><i class='fa-solid fa-pencil'></i></a>{$dagRestrictText}{$roleRestrictText}",
        ...
        elseif ($type == 'pdf_archive')
        {
            ...
            foreach ($files as $file)
            {
                ...
                $row[] = RCView::a(['style'=>'text-decoration:underline;padding:0 5px;font-size:14px;', 'title'=>$lang['docs_96'], 'href'=>APP_PATH_WEBROOT . "DataEntry/index.php?pid=".$Proj->project_id."&instance={$file['instance']}&event_id={$file['event_id']}&page=$form_name&id={$file['record']}"],
                    $file['record'] . " $record_extra_label"
                );
                // Survey
    ...
    public static function renderIndexPage()
    {
        ...
        addLangToJS(['period', 'docs_74', 'sendit_32', 'docs_77', 'docs_78', 'docs_79', 'docs_80', 'docs_81', 'docs_82', 'sendit_03', 'sendit_04', 'sendit_05', 'global_01',
            ...
            'docs_1145', 'docs_1146', 'docs_1147', 'global_03', 'docs_1149', 'docs_1150', 'docs_1152', 'docs_1153', 'econsent_181']);
