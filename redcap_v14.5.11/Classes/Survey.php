...
class Survey
{
	...
	public static function getSurveyQueueForRecord($record, $returnTrueIfOneOrMoreItems=false, $project_id=null)
	{
		...
		$record_data = Records::getData(['project_id'=>$Proj->project_id, 'records'=>$record, 'fields'=>$fields, 'events'=>$events, 'returnEmptyEvents'=>true]);
	...
	public static function displaySurveyQueueForRecord($record, $isSurveyAcknowledgement=false, $justCompletedSurvey=false)
	{
		...
		$table_title = 	RCView::div(array('style'=>''),
							RCView::div(array('style'=>'float:left;color:#800000;font-size:14px;'),
								...
								RCView::span(array('style'=>'vertical-align:middle;'), RCView::tt("survey_505"))
							) .
			...
			RCView::div(array('title'=>''), // prevent tooltip from showing all over the place
					...
					RCView::div(array('style'=>'margin-bottom:20px;'),
						...
						RCView::div(array('style'=>'margin:5px 0 10px 25px;'),
							...
							($participant_email != '' ? '' :
								RCView::div(array('style'=>'color:#800000;font-size:11px;margin:5px 10px 0;'), '* '.RCView::tt("survey_1339"))
							)
						)
					)
				)
	...
	public static function getFollowupSurveyParticipantIdHash($survey_id, $record, $event_id=null, $forceInsert=false, $instance=null, $projectIdOverride=null)
	{
        ...
        <script type="text/javascript">
            ...
            function checkComposeForSurveyLink(ob) {
                ...
                while (matches = regex.exec(text)) {
                    if (starts_with(matches[0], app_path_webroot_full+'surveys/') && matches[0].indexOf('__file=') === -1) {
	...
	public static function renderEconsentPdfFrame()
	{
		...
		if ($typeText != '') 	$versionTypeTextArray[] = ($header_translations["data_entry_429"] ?? RCView::getLangStringByKey("data_entry_429"))." ".$typeText;
	...
	public static function exitSurvey($text, $largeFont=true, $closeSurveyBtnText=null, $justCompletedSurvey=false)
	{
		global $text_to_speech, $font_family, $text_size, $theme, $custom_theme_attr, $fetched, $Proj;
		...
		if ($closeSurveyBtnText !== false) {
			...
			print 	RCView::div(array('style'=>'padding:10px 10px 0;'),
				RCView::button(array('onclick'=>"
							...
							mlm_lang = mlm_lang ? '&".MultiLanguage::LANG_GET_NAME."=' + mlm_lang : '';
	...
	public static function getParticipantList($survey_id, $event_id=null, $recordsAllowlist=array(), $participantIdsAllowlist=array())
	{
        ...
        if ($performResponseTableCheck)
        {
			...
			$sql = "select distinct d.record, ifnull(d.instance, 1) as instance 
                from ".\Records::getDataTable(PROJECT_ID)." d 
				left join redcap_surveys_participants p 
				    on p.event_id = d.event_id and p.survey_id = $survey_id and p.participant_email is not null
				left join redcap_surveys_response r 
				    on r.participant_id = p.participant_id and r.record = d.record and ifnull(d.instance, 1) = r.instance
				where d.project_id = " . PROJECT_ID . " and d.event_id = $event_id and d.field_name = '{$Proj->surveys[$survey_id]['form_name']}_complete' 
				    and r.response_id is null $armSql $recordSql";
			if (!$isRepeatingFormOrEvent) {
...
//                        ) on p.event_id = x.event_id and p.survey_id = $survey_id and r.participant_id = p.participant_id and r.record = x.record and p.participant_email is not null
//                    where d.record is null and r.response_id is null";
	...
	public static function sendSurveyConfirmationEmail($survey_id, $event_id, $record, $respondent_email_override=null, $instance=1)
	{
		...
		if ($survey_attr['confirmation_email_subject'] != '' && $survey_attr['confirmation_email_content'] != '')
		{
			...
			if (!empty($respondent_email))
			{
				...
				$type_prompt = MultiLanguage::getUITranslation($context, "data_entry_429"); // Type:
				...
				if ($survey_attr['confirmation_email_attach_pdf'] == '1') {
					...
					if ($typeText != '') 	$versionTypeTextArray[] = $type_prompt." ".$typeText;
					$versionTypeText = implode(', ', $versionTypeTextArray);
    ...
    public static function redirectIfCurrentInstanceHasData($project_id, $record, $form_name, $event_id, $instance, $redirectToForm=false)
    {
        ...
        if (!isset($Proj->forms[$form_name]['survey_id']) || !isinteger($instance) || !$Proj->isRepeatingFormOrEvent($event_id, $form_name)) {
            return false;
        }
        // Does the current instance have [survey] data (i.e., some data was entered via survey)?
        $sql = "select 1 from ".\Records::getDataTable($project_id)." where project_id = $project_id and event_id = $event_id
                and record = '".db_escape($record)."' and field_name != '{$Proj->table_pk}'
                and instance " . ($instance > 1 ? "= $instance" : "is null");
        $q = db_query($sql);
        $instrumentHasData = (db_num_rows($q) > 0);
        if (!$instrumentHasData) {
            return false;
        }
    ...
    public static function promptSurveyCode($voiceCall, $respondentPhoneNumber, $project_id)
    {
        ...
        if ($voiceCall) {
            ...
            // Say the field label
            $gather->say($lang['survey_619']." ".$lang['survey_1308']);
            // Output twiml
            print $twiml;
        } else {
            ## SMS
            $messaging = new Messaging($project_id);
            $smsSuccess = $messaging->send($lang['survey_619']." ".$lang['survey_1308'], $respondentPhoneNumber);
            if ($smsSuccess !== true) {
                // Error sending SMS
            }
        }
