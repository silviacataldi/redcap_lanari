...
class DataExport
{
                                    ...
                                    $usedForSmartChart=false, $returnOnlyFileFields=false, $cacheManager=null, $cacheOptions=null)
	{
		...
		if (!$bypassReportAccessCheck && ((defined('SUPER_USER') && !SUPER_USER) || !defined('SUPER_USER'))) {
		...
		if (($isDataExportAction && isset($user_rights['forms_export']) && !empty($user_rights['forms_export']) && !(defined("SUPER_USER") && SUPER_USER))
		...
		if ($liveFilterLogic != "") {
			if ($report['limiter_logic'] != '') {
                $report['limiter_logic'] = LogicParser::removeComments($report['limiter_logic']);
    ...
    public static function saveShortUrl($hash, $custom_url, $report_id)
    {
        ...
        if ($GLOBALS['enable_url_shortener_redcap']) {
            ...
            $shorturl = Survey::getCustomShortUrl(APP_PATH_SURVEY_FULL . "index.php?__report=" . $hash, $custom_url);
            if ($shorturl == '') exit('0');
        }
