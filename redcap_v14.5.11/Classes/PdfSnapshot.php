...
class PdfSnapshot
{
    ...
    {
        $sql = "select consent_id from redcap_pdf_snapshots 
                where trigger_surveycomplete_survey_id = ? and active = 1
                limit 1";
        $q = db_query($sql, $survey_id);
        if (db_num_rows($q) === 0) return false;
        // If snapshot enabled and is governed by e-Consent, also check if e-Consent is enabled for this survey
        $consent_id = db_result($q) ?? null;
        if (isinteger($consent_id)) {
            $sql = "select 1 from redcap_econsent 
...
//        }
                                                 ...
                                                 $compactPDF=true, $surveyCompletionTrigger=false, $snapshot_id=null)
    {
        ...
        if ($econsentEnabledForSurvey) {
            ...
            if ($typeText != '') 	$versionTypeTextArray[] = (!$translatePdf ? RCView::getLangStringByKey("data_entry_429") : MultiLanguage::getUITranslation($context, "data_entry_429"))." ".$typeText; //= Type:
        }
        ...
        if ($consent_id == null && empty($selected_forms_events)) {
            // Store whole record
            $pdf_contents = REDCap::getPDF($record, null, null, false, $pdfInstance, $compactPDF, "", "", false, true);
        } else {
            ...
            $pdf_contents = REDCap::getPDF($record, $pdfInstrument, $pdfEventId, false, $pdfInstance, $compactPDF, "", "", false, true, false, $selected_forms_events_array);
        }
    ...
    public function manualTriggerSave($snapshot_id, $record, $event_id=null, $form=null, $instance=1)
    {
        ...
        $formViewingRight = $user_rights['forms'][$form] ?? 0;
        if ($isSurveyCompletionTrigger && $formViewingRight != '1' && $formViewingRight != '3') { // 1 and 3 = edit form rights
            return [0,0];
        }
        // Attributes
        $saveToField = $attr['pdf_save_to_field'] ?? false;
