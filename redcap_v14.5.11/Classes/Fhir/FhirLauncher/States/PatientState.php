...
class PatientState extends State
{
	...
	public function run() {
		...
		if(!$patientData || $patientData->ID !== $patientID) {
			...
			$patientData = $this->getPatientData($patientID, $user_id, $accessTokenDTO->access_token);
		...
		$navbar = $renderer->run('partials.navbar', compact('session', 'patientData', 'username', 'fhirUser'));
		$footer = $renderer->run('partials.alerts', [
			'alerts' => SessionDataUtils::getAlerts(),
		]);
		$ddpExplanationDialogContent = DynamicDataPull::getExplanationText($isFhir=true);
		$html = $main = $renderer->run(
			'patient',
			compact(
				'navbar',
				'patientData',
	 ...
	 */
	private function getPatientData($patient_id, $user_id, $accessToken)
	{
		...
		/** @var Patient $patient */
		$patient = $response->resource ?? null;
		if(!$patient) throw new Exception("Error: could not retrieve patient data", 1);
