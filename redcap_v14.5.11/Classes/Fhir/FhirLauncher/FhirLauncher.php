...
class FhirLauncher
{
	...
	private function detectState() {
		...
		// order is important
		// make sure there is a state at all times
		if(!isset($_GET[self::FLAG_SESSION_ID]) ?? false) { $state = new NoState($this); }
		// else if($_GET[self::FLAG_REGISTER_USER] ?? false) { $state = new RegisterUserState($this); }
		else if($_GET[self::FLAG_STORE_TOKEN] ?? false) { $state =  (new StoreTokenState($this))->add($authMiddleware); }
		...
		// Logging::writeToFile(APP_PATH_TEMP.'EHR URIs.log', $actual_link);
		// END DEBUG
