...
class LogicParser {
	 ...
	 */
	public function parse($str, $eventNameToId=null, $createFunction=true, $isCalcField=false, $useAnonFunction=false, $bypassFormatLogicToPHP=false, $bypassFunctionCache=false)
	{
		$str = LogicParser::removeComments($str);
        ...
        if (!$bypassFormatLogicToPHP) {
			$str = LogicTester::formatLogicToPHP($str);
		} else {
			...
			if ($createFunction) {
				...
				if ($isCalcField) {
					...
					if (strpos($code, "+") !== false) {
						$hasConcat = (strpos($code, "concat") !== false && preg_match("/(concat)(\s*)(\()/", $code));
                        $hasIf = (strpos($code, "if") !== false && preg_match("/(if)(\s*)(\()/", $code));
