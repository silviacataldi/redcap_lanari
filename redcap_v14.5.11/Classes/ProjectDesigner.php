...
class ProjectDesigner
{
	 ...
	 * @return void
	 */
	public function createForm($form_name, $afterForm=null, $label=null) {
	...
	public function deleteForm($form_name)
	{
		...
		if ($randomization && Randomization::setupStatus())
		{
			...
			$sql = "SELECT 1 FROM $metadata_table WHERE project_id = $project_id AND form_name = '{$form_name}'
	...
	public function deleteField($fieldName, $form_name, $sectionHeader=false)
	{
		$project = $this->project;
		$project_id = $project->project_id;
		$status = $this->getProjectStatus();
		...
		// Delete field from metadata table
			...
			if ($randomization && Randomization::setupStatus())
			{
				...
				if ($fieldName == $randAttr['targetField'] || in_array($fieldName, array_keys($randAttr['strata']))) {
