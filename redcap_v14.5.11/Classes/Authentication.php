...
class Authentication
{
	...
	public static function authenticate()
	{
		...
		elseif ($auth_meth_global == 'oauth2_azure_ad') {
            ...
            if (!(isset($_SESSION['redcap_userid'])) && ($ehrRedirectURL = self::isEhrContext())) {
				$_SESSION['redcap_page'] = $ehrRedirectURL;
			}
			// auth via azure ad
        ...
        elseif ($auth_meth_global == 'oauth2_azure_ad_table' && ($_GET['action']??"" != 'passwordreset')) {
            ...
            if ($userid === null && !isset($_GET['code']) && !isset($_GET['state']) && !isset($_GET['iss']) && !(isset($_GET['action']) && $_GET['action'] == 'oauth2_azure_ad_login')) {
                $redirectUrl = $_SERVER['REQUEST_URI'];
                savecookie('auth_redirect_url', encrypt($redirectUrl), 120);
            } else if ($userid === null && ($ehrRedirectURL = self::isEhrContext())) {
				$_SESSION['redcap_page'] = $ehrRedirectURL;
				// will redirect back to the SMART on FHIR auth process
                savecookie('auth_redirect_url', encrypt($ehrRedirectURL), 120);
			}
            if (isset($userid) && $userid != null && !isset($_GET['code']) && !isset($_GET['state'])) {
                // Do nothing
		...
		if (UserRights::isImpersonatingUser()) {
			...
			$GLOBALS['allow_create_db'] = $userInfoImpersonated['allow_create_db'];
		}
	...
	static function setLastLoginTime($userid, $addLoginEvent=false)
	{
		...
		$sql = "update redcap_user_information i, (select ifnull(min(ts), '".NOW."') as ts from redcap_log_view 
				...
				where i.username = '" . db_escape($userid) . "' and l.ts is not null";
		$q = db_query($sql);
		...
		if (isset($_GET['logout']) && $_GET['logout'])
		{
			...
			elseif ($auth_meth_global == 'openid_connect' || ($auth_meth_global == 'openid_connect_table' && !self::isTableUser($currentUser))) {
                try {
                    ...
                    $oidc->signOut($openid_connect_id_token, APP_PATH_WEBROOT_FULL."index.php?logout=1");
                } catch (Exception $e) { }
			}
	...
	/**
	 * determine if we have been redirected here from a
	 * SMART on FHIR authentication process
	 *
	 * @return string|false
	 */
	private static function isEhrContext() {
			...
			// State validation 
				...
				try {
                    ...
                    if (isVanderbilt() || isDev()) {
                        self::convertVanderbiltUsername($userinfo);
                    }
    ...
    // Vanderbilt only: Convert legacy VUMCID/VUnetID (onPremisesSamAccountName) to UPN (email address) in relevant db tables
    private static function convertVanderbiltUsername($authinfo)
    {
        ...
        $newUsername = $authinfo['userPrincipalName'] ?? null;
        ...
        foreach ($tablesToChange as $table=>$col) {
            ...
            db_query($sql, [$newUsername, $oldUsername]);
        }
        ...
        // Add logged event
        Logging::logEvent($sql,"redcap_user_information","MANAGE",$newUsername,"old_username = '$oldUsername', new_username: '$newUsername'",$log);
    }
    ...
    private static function initOpenIDConnect()
    {
        try {
            ...
            if (isset($configParams['scopes_supported']) && is_array($configParams['scopes_supported']) && !empty($configParams['scopes_supported'])) {
            ...
            if ($success) {
                ...
                if ($userinfo['user_email'] == '')
                {
                    ...
                    $sql = "replace into redcap_user_information (username, user_email, user_firstname, user_lastname, user_firstvisit,
                            ...
                            '".db_escape($userid)."', '".db_escape($oidc->getVerifiedClaims('email'))."', '".db_escape($oidc->getVerifiedClaims('given_name'))."',
                            '".db_escape($oidc->getVerifiedClaims('family_name'))."', '".NOW."', '".db_escape($GLOBALS['allow_create_db_default'])."', '".NOW."', '".db_escape($GLOBALS['default_datetime_format'])."',
