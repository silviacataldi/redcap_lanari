...
class DataEntry
{
	...
	public static function renderRecordHomePage()
	{
		...
		if (isset($_GET['id']))
		{
			...
			while ($row = db_fetch_array($q)) {
				$this_lock_ts = " <img class='gridLockIcon' src='".APP_PATH_IMAGES."lock_small.png' title='".js_escape($lang['bottom_59'])." ".DateTimeRC::format_ts_from_ymd($row['timestamp'])."'>"; // ttfy
				...
				if ($hasRepeatingForms && $Proj->isRepeatingForm($row['event_id'], $row['form_name']) && isset($locked_forms_grid[$row['event_id'].",".$row['form_name'].",".$row['instance']])) {
					$locked_forms_grid[$row['event_id'].",".$row['form_name'].",".$row['instance']] = " <img class='gridLockIcon' src='".APP_PATH_IMAGES."locks_small.png' title='".js_escape($lang['data_entry_283'])."'>"; // ttfy
				}
			...
			$q = db_query($qsql);
			while ($row = db_fetch_array($q)) {
				$this_esign_ts = " <img class='gridEsignIcon' src='".APP_PATH_IMAGES."tick_shield_small.png' title='".js_escape($lang['data_entry_224'])." ".DateTimeRC::format_ts_from_ymd($row['timestamp'])."'>"; // ttfy
                ...
                if ($GLOBALS['esignature_enabled_global']) {
                    if ($hasRepeatingForms && $Proj->isRepeatingForm($row['event_id'], $row['form_name']) && isset($esigned_forms_grid[$row['event_id'].",".$row['form_name'].",".$row['instance']])) {
                        $esigned_forms_grid[$row['event_id'].",".$row['form_name'].",".$row['instance']] = " <img class='gridEsignIcon' src='".APP_PATH_IMAGES."tick_shields_small.png' title='".js_escape($lang['data_entry_284'])."'>"; // ttfy
                    }
								...
								RCView::div(array('id'=>'recordActionDropdownDiv', 'style'=>'display:none;position:absolute;z-index:1000;'),
									RCView::ul(array('id'=>'recordActionDropdown'),
										...
										($user_rights['data_export_tool'] == '0' ? '' :
											RCView::li(array(),
												RCView::a(array('target'=>'_blank', 'style'=>'display:block;', 'href'=>APP_PATH_WEBROOT."index.php?route=PdfController:index&pid=$project_id&id=".RCView::escape($_GET['id'])),
										...
										($user_rights['data_export_tool'] == '0' ? '' :
											RCView::li(array(),
												RCView::a(array('target'=>'_blank', 'style'=>'display:block;', 'href'=>APP_PATH_WEBROOT."index.php?route=PdfController:index&pid=$project_id&id=".RCView::escape($_GET['id'])."&compact=1"),
													RCView::span(array('style'=>'vertical-align:middle;color:#A00000;'),
										...
										// Lock record
										(!($user_rights['lock_record_multiform'] && !$wholeRecordIsLocked) ? '' :
											RCView::li(array(),
			...
			?>
			<style type="text/css">
			table.dataTable tbody tr td { padding:3px !important; vertical-align:top; }
			table.dataTable tr td a { text-decoration:none; }
			table.dataTable .surveyLabel { margin:0 4px;color:#888;font-size:10px;font-family:tahoma; }
			table.dataTable .gridLockEsign { text-align:center; }
			table.dataTable .labelform { font-weight:normal;line-height:18px; }
			...
			***************************************************************/
			...
			$sql = "select d.event_id, d.field_name, d.value, d.instance from ".\Records::getDataTable($project_id)." d 
					...
					and d.field_name in ('".implode("_complete', '", array_keys($Proj->forms))."_complete')";
			if ($longitudinal && isset($Proj->events[$arm])) {
			...
			if ($existing_record && (($DDP->isEnabledInSystem() && $DDP->isEnabledInProject()) || ($DDP->isEnabledInSystemFhir() && $DDP->isEnabledInProjectFhir()))) {
				?>
				<script type="text/javascript">
				var record_exists = 1;
				</script>
				<?php $DDP->renderJsAdjudicationPopup($true_record); 
				if (isset($_GET['openDDP'])) { ?>
				<script type="text/javascript">
				$(function() { openAdjudicationDialog(getParameterByName('id')); });
				</script>
				<?php }
			}
			...
			foreach ($Proj->events[$arm]['events'] as $this_event_id=>$this_event) 
			{
				...
				foreach ($this_event_instances as $this_instance)
				{
					...
					if ($existing_record && $is_repeating_event && $this_instance == $max_event_instance) {
						...
						$addInstanceBtn .= "<div class='divBtnAddRptEv'><button $repeatEventBtnDisabled onclick=\"gridAddRepeatingEvent(this);\" event_id='$this_event_id' instance='$this_instance' class='btn btn-xs btn-defaultrc btnAddRptEv nowrap'>+&nbsp;".RCView::tt("data_entry_247")."</button></div>";
					}
					// Event label piping
					$custom_event_label = "";
					if ($custom_event_labels_all != "") {
						$custom_event_label = DataEntry::getRecordCustomEventLabel($id, $this_event_id, $this_instance);
			...
			foreach ($Proj->forms as $form_name=>$attr)
			{
				...
				foreach ($form_events as $this_event_id=>$eattr)
				{
                    ...
                    $formsAccess = FormDisplayLogic::getAccess('record_home_page', $true_record, $this_event_id);
					...
					foreach ($grid_form_status[$row['event_id']][$row['form_name']] as $this_instance=>$this_form_status) 
					{
						...
						if (in_array($row['form_name'], $eattr))
						{
							...
							if ($form_has_multiple_instances) {
								$this_url = 'javascript:;';
								$onclick = "onclick=\"loadInstancesTable(this,'".js_escape($true_record)."', {$row['event_id']}, '{$row['form_name']}');\"";
							} else {						
								$onclick = "";
							}
                            // Form Display Logic
                            $link_style = "";
							if (isset($formsAccess[$true_record][$row['event_id']][$row['form_name']]) && $formsAccess[$true_record][$row['event_id']][$row['form_name']] != 1) {
                                $link_style = 'style="pointer-events: none; opacity: 0.2;"';
                            }
							$tr .= "<a href='$this_url' $onclick $link_style>$this_button</a>";
							...
							if ($existing_record && $event_has_repeating_forms) {
								...
								// Display button
								$tr .= "<button data-rc-lang-attrs='title=grid_43' title='".js_escape($lang['grid_43'])."' onclick=\"window.location.href='$thisPlusBtnUrl&instance=$next_instance';\" class='btn btn-defaultrc btnAddRptEv ".($is_repeating_form && $this_color != "circle_gray.png" ? "opacity50" : "invis")."' $link_style>+</button>";
							}
							//Display lock icon for any forms that are locked for this record
							if ((isset($locked_forms_grid[$row['event_id'].",".$row['form_name'].",".$this_instance]) || isset($esigned_forms_grid[$row['event_id'].",".$row['form_name'].",".$this_instance]))) 
							{
								$tr .= RCView::div(array('class'=>'gridLockEsign nowrap'), 
			...
			if ($longitudinal && $existing_record && $user_rights['record_delete'] && !$wholeRecordIsLocked)
			{
				...
				foreach ($deleteRow as $attr)
				{
					...
					if ($grayIconsByEvent[$attr['event_id']]['form_count'] > $grayIconsByEvent[$attr['event_id']]['gray_count']) {
						...
						// Set cell content
						$tdLink = "<a href='javascript:;' onclick=\"deleteEventInstance({$attr['event_id']},{$attr['instance']},$isRepeatingEvent);\" style='color:#A00000;'><span class='fas fa-times opacity35' style='padding:2px 5px;' {$deleteEventRcLangAttrs} title='{$deleteEventText}'></span></a>";
					}
					// Find collapsed state of column
		...
		else
		{
			...
			{
				if ($_GET['msg'] == 'edit') {
					print "<div class='darkgreen' style='margin:10px 0;'><img src='".APP_PATH_IMAGES."tick.png'> ".RCView::escape($table_pk_label)." <b>".RCView::escape($_GET['edit_id'])."</b> {$lang['data_entry_08']}</div>";
				}
				elseif ($_GET['msg'] == 'edit_no_next') {
					print "<div class='darkgreen' style='margin:10px 0;'>
							<img src='".APP_PATH_IMAGES."tick.png'> ".RCView::escape($table_pk_label)." <b>".RCView::escape($_GET['edit_id'])."</b> {$lang['data_entry_08']}{$lang['period']}<br>
							<div style='font-weight:bold;margin-top:7px;'>{$lang['data_entry_411']}</div>
						   </div>";
				}
			}
			...
			if ($status < 1) {
				print  "<div class='yellow' style='width:90%;max-width:700px;margin:15px 0 25px;'>
							<img src='".APP_PATH_IMAGES."exclamation_orange.png'>
							<span style='font-size:14px;'>".RCView::tt("data_entry_532")."</span></div>";
			}
			?>			
			<style type="text/css">
    ...
    public static function renderForm($elements, $element_data=array(), $hideFields=array())
    {
        ...
        if (($isRepeatingFormOrEvent || $longitudinal) && PAGE == 'DataEntry/index.php' && isset($_GET['id']))
        {
            ...
            foreach ($surveyEmailInvitationFields as $surveyEmailInvitationField) {
                $surveyEmailInvitationFieldsForm = $Proj->metadata[$surveyEmailInvitationField]['form_name'];
        ...
        if ($isSurveyPage || ((PAGE == "DataEntry/index.php") && isset($_GET['id'])))
        {
                    ...
                    foreach ($these_strings as $this_key=>$this_string)
                    {
                        ...
                        if ($this_string != '' && strpos($this_string, '[') !== false && strpos($this_string, ']') !== false)
                        {
                            ...
                            foreach (array_merge($possibleVars, $specialPipingTags) as $this_field) {
                                ...
                                if (!isset($Proj->metadata[$this_field]) && !$isSmartVar) continue;
            ...
            if (!empty($elements_replace))
            {
                ...
                foreach ($piping_transmitter_fields as $this_field) {
                    ...
                    if ($this_type == 'calc' || ($this_type == 'text' && (Calculate::isCalcDateField($Proj->metadata[$this_field]['misc']) || Calculate::isCalcTextField($Proj->metadata[$this_field]['misc'])))) {
                ...
                function updatePipingDropdowns(transmitter_field,transmitter_field_value) {
                    ...
                    $.post('<?=$piping_dropdown_replace_page?>',{ formdata: JSON.stringify($('#form').serializeObject()), page: getParameterByName('page'), record: '<?=Survey::$nonExistingRecordPublicSurvey ? '' : js_escape($_GET['id'])?>', event_id: event_id, instance: '<?=js_escape($_GET['instance'])?>', json_piping_receiver_dropdown_fields: json_piping_receiver_dropdown_fields, transmitter_field: transmitter_field, transmitter_field_value: transmitter_field_value, lang: lang },function(data){
                        if (data != '') {
                            ...
                            // Trigger the select's change in order to propagate piping further
        ...
        if ($isSurveyPage || ((PAGE == "DataEntry/index.php") && isset($_GET['id'])))
        {
            ...
            if ($randomizationEnabled)
            {
                ...
                // Determine if this record has already been randomized (but only if it already exists)
                $wasRecordRandomized = ($hidden_edit == 1) ? Randomization::wasRecordRandomized($_GET['id']) : false;
                ...
                if ($wasRecordRandomized)
                {
                    ...
                    if (!empty($randomizationCriteriaFieldList))
                    {
                        ?>
                        <script type="text/javascript">
                        var randomizationCriteriaFieldList = new Array(<?=prep_implode($randomizationCriteriaFieldList)?>);
                        </script>
                        <?php
                    }
                }
        ...
        if ((PAGE == "DataEntry/index.php") && isset($_GET['id']))
        {
            ...
            if ($displayEsignature)
            {
                ...
                $esignature_text = "<div id='esignchk'>
                                        <input type='checkbox' id='__ESIGNATURE__' $onclick" . ($is_esigned ? " checked" : "") . ($is_esigned || $wholeRecordIsLocked ? " disabled" : "") . ">
                                        <img src='" . APP_PATH_IMAGES . "tick_shield.png'> ".RCView::tt("global_34")." &nbsp;<span style='color:#444;font-weight:normal;'>(<a style='text-decoration:underline;font-size:10px;font-family:tahoma;'
            ...
            if ($Proj->isRepeatingForm($_GET['event_id'], $_GET['page']))
            {
                ...
                $instanceMax = empty($instanceList) ? null : max(array_keys($instanceList));
                if ($instanceCount > 0)
                {
                    ...
                    foreach ($instanceList as $this_instance=>$this_form_status)
                    {
                        ...
                        $repeatInstanceDropdownChoices .= RCView::li(array('style'=>$instanceBgColor),
                                                            RCView::a(array('href'=>$instanceChoiceBaseUrl.$this_instance, 'style'=>'display:block;text-decoration:none;'),
                                                                "<span style='vertical-align:middle;'>".RCView::img(array('src'=>RepeatInstance::getStatusIcon($this_form_status), 'alt'=>RepeatInstance::getStatusIconAltText($this_form_status))) .
                                                                "<span style='color:#800000;'><span style='margin:0 2px;'>$this_instance</span>".
                                                                "</span>$pipedLabel</span>"
                                                            )
                                                        );
                    }
                    // Add "add new instance" choice to drop-down (if we're not already on it)
                    if ($_GET['instance'] <= $instanceMax) {
                        $repeatInstanceDropdownChoices .= RCView::li(array('style'=>'border-top:1px solid #eee;padding:5px 10px;'),
                                                            RCView::button(array('class'=>'btn btn-defaultrc btnAddRptEv', 'onclick'=>"window.location.href='".$instanceChoiceBaseUrl.($instanceMax+1)."';return false;",
                                                                'style'=>'padding:0px 4px !important;margin:0 2px 2px 0;font-size:12px !important;'), ' + ' . RCView::tt("data_entry_247"))
                                                        );
                    }
                    // Is this a partially/completed response?
                    $formStatus = $element_data[$_GET['page']."_complete"];
                    if (!isset($instanceList[$_GET['instance']])) {
                        $formStatus = '';
                    } elseif ($_GET['instance'] > $instanceMax) {
                        $formStatus = '';
                        $instanceMax++;
                    } elseif (isset($Proj->forms[$_GET['page']]['survey_id'])) {
                        $responseCompleted = Survey::isResponseCompleted($Proj->forms[$_GET['page']]['survey_id'], addDDEending($_GET['id']), $_GET['event_id'], $_GET['instance']);
                    ...
                    $pipedLabel = "";
                    if ($pre_piped_label != "") {
                        $pipedLabel = trim(Piping::replaceVariablesInLabel($pre_piped_label, addDDEending($_GET['id']), $_GET['event_id'], $_GET['instance'], null,
                                            true, null, false, $_GET['page'], 1, false, false, $_GET['page']));
                        if ($pipedLabel != "") $pipedLabel = "<span style='color:#999;margin:0 4px 0 2px;'>&ndash;</span>$pipedLabel";
                    }
                    // Output the drop-down
                    $repeatInstanceDropdown = RCView::div(array('style'=>'float:left;', 'class'=>'d-print-none'),
                                                RCView::tt("grid_46", "span", array('style'=>'font-size:13px;margin-right:5px;')) .
                                                RCView::button(array('id'=>'repeatInstanceDropdown', 'onclick'=>"showBtnDropdownList(this,event,'repeatInstanceDropdownDiv');return false;", 'class'=>'jqbuttonmed'),
                                                    RCView::img(array('src'=>RepeatInstance::getStatusIcon($formStatus), 'alt'=>RepeatInstance::getStatusIconAltText($formStatus), 'style'=>'vertical-align:middle;margin-left:2px;')) .
        ...
        elseif (PAGE == "Design/online_designer.php" || PAGE == "Design/online_designer_render_fields.php")
        {
            ...
            if (isset($_GET['page']))
            {
                ...
                $bookend1 = '<td class="frmedit_row" style="padding:0 10px;background-color:#ddd;">
                        ...
                        <tr>';
                $bookend2 = '</tr>{action_tags}</table></td>';
        ...
        if ($isSurveyPage && !(isset($GLOBALS['public_survey']) && $GLOBALS['public_survey'] && Survey::$nonExistingRecordPublicSurvey) && isset($_GET['event_id']))
        {
            ...
            if (isset($lockingWhole->lockedWhole[$_GET['id']]) || db_num_rows(db_query($sql))) {
                ...
                $(function(){
                    ...
                    $('form#form').before('<?=js_escape(RCView::div(array('class'=>'yellow', 'style'=>'max-width:97%;'), RCView::img(array( 'src'=>'exclamation_orange.png')) . RCView::tt("survey_674")))?>');
                });
                </script>
                <style type="text/css">
                </style>
        ...
        $survey_id = $Proj->forms[$_GET['page']]['survey_id'] ?? null;
            ...
            && isset($Proj->forms[$_GET['page']]['survey_id']) && $hidden_edit == 1 && isset($Proj->forms[$_GET['page']]['survey_id']))
        {
            ...
            if (db_num_rows($q) > 0)
            {
                ...
                if ($enable_edit_survey_response && $survey_completion_time != "" && $user_rights['forms'][$_GET['page']] == '3')
                {
                    ?>
                    <script type='text/javascript'>
                    $(function(){
                ...
                if ($repeatInstanceDropdown != "") {
                    print "<div id='formtop-div-2' class='formtop-div d-print-none' style='display:block;'>$repeatInstanceDropdown<div class='clear'></div></div>";
                }
                ...
                if ($survey_completion_time != "" && $user_rights['forms'][$_GET['page']] == '3' && $enable_edit_survey_response
                ...
                <div class="d-print-none <?=$survey_completion_time == "" ? "orange" : "darkgreen"?>" id="form_response_header">
                    <?php if ($econsentCompleteAndReadonly) { ?>
                        ...
                        ?>
                    <?php } elseif ($user_rights['forms'][$_GET['page']] == '3' && $enable_edit_survey_response && !$isPromisInstrument) { ?>
                    ...
                    if ($secondary_pk != '' && $GLOBALS['secondary_pk_display_value'])
                    {
                        ...
                        if ($secondary_pk_val != '') {
                            ...
                            if ($GLOBALS['secondary_pk_display_label']) {
                                print "<span data-mlm-secondary_pk='{$secondary_pk}'>{$Proj->metadata[$secondary_pk]['element_label']}</span> ";
                            }
        ...
        $enhanced_choices = (($isSurveyPage && isset($_GET['page']) && $Proj->surveys[$Proj->forms[$_GET['page']]['survey_id']]['enhanced_choices'])
        ...
        if ($GLOBALS['require_change_reason'] && PAGE == 'DataEntry/index.php' && isset($_GET['id'])) {
        ...
        foreach ($elements as $rr_key=>$rr_array)
        {
            ...
            if ($isFormOrSurveyPage && $name != $Proj->table_pk)
            {
                $calcChangesClass = "";
                // @IF ACTION TAG
                if (isset($Proj->metadata[$name]['misc']) && strpos($Proj->metadata[$name]['misc'], "@IF") !== false) {
                    $replacedMisc = Form::replaceIfActionTag($Proj->metadata[$name]['misc'], $Proj->project_id, addDDEending($_GET['id']), $_GET['event_id'], $_GET['page'], $_GET['instance']);
                    if ($replacedMisc != $Proj->metadata[$name]['misc']) {
                        $Proj->metadata[$name]['misc_original'] = $Proj->metadata[$name]['misc'];
                        $Proj->metadata[$name]['misc'] = $replacedMisc;
                        ...
                        if ($replacedMisc == "") {
                            ...
                            preg_match_all($action_tags_regex, $Proj->metadata[$name]['misc'], $this_misc_match);
                ...
                if (in_array('@CHARLIMIT', $rr_array['action_tags'])) {
                    $actionTagLimitGoal = Form::getValueInActionTag($Proj->metadata[$name]['misc'], "@CHARLIMIT");
                    ...
                    $actionTagLimitGoal = Form::getValueInActionTag($Proj->metadata[$name]['misc'], "@WORDLIMIT");
                    if (is_numeric($actionTagLimitGoal) && $actionTagLimitGoal > 0) {
                ...
                if (in_array('@HIDECHOICE', $rr_array['action_tags']))
                {
                    ...
                    $hideChoiceText = Form::getValueInQuotesActionTag($Proj->metadata[$name]['misc'], "@HIDECHOICE");
                ...
                if (in_array('@SHOWCHOICE', $rr_array['action_tags']))
                {
                    ...
                    $showChoiceText = Form::getValueInQuotesActionTag($Proj->metadata[$name]['misc'], "@SHOWCHOICE");
                ...
                if (isset($rr_array['action_tags']) && isset($Proj->metadata[$name]['element_type']) && $Proj->metadata[$name]['element_type'] == 'checkbox' && in_array('@NONEOFTHEABOVE', $rr_array['action_tags']))
                {
                    // Obtain the NONEOFTHEABOVE text for this field (try with quotes and no quotes)
                    $noneOfTheAboveChoiceTextNoQuotes = Form::getValueInActionTag($Proj->metadata[$name]['misc'], "@NONEOFTHEABOVE");
                    $noneOfTheAboveChoiceText = ($noneOfTheAboveChoiceTextNoQuotes == "") ? Form::getValueInQuotesActionTag($Proj->metadata[$name]['misc'], "@NONEOFTHEABOVE") : $noneOfTheAboveChoiceTextNoQuotes;
                    if ($noneOfTheAboveChoiceText != "") {
                        // If a checkbox, then parse the comma-delimited values into an array
                        $currentChoices = $normalChoices = parseEnum($Proj->metadata[$name]['element_enum']);
            ...
            if (($isSurveyPage || PAGE == 'DataEntry/index.php') && isset($_GET['id']) && in_array('@DEFAULT', $rr_array['action_tags'])
                ...
                && $Proj->metadata[$name]['element_type'] != 'file')
            {
                // Obtain the default value for this field
                $defaultValuePre = Form::getValueInQuotesActionTag($Proj->metadata[$name]['misc'], "@DEFAULT");
                ...
                if ($value == '' && $defaultValue != '' && ($hidden_edit == 0 || $pageHasData === false)) {
                    ...
                    if ($setValuesChangedFlag) { ?><script type="text/javascript">dataEntryFormValuesChanged = true;</script><?php }
            ...
            if (($isSurveyPage || PAGE == 'DataEntry/index.php') && isset($_GET['id']) && (in_array('@SETVALUE', $rr_array['action_tags']) || in_array('@PREFILL', $rr_array['action_tags']))
                ...
                // Obtain the default value for this field
                $pipeValuePre = Form::getValueInQuotesActionTag($Proj->metadata[$name]['misc'], (in_array('@SETVALUE', $rr_array['action_tags']) ? "@SETVALUE" : "@PREFILL"));
                // Perform piping on the default value (if needed)
                    ...
                    ))
                {
                    ...
                    if ($randomizationField == $name && $randomizationEvent == $_GET['event_id'])
                    {
                        ...
                        if ($wasRecordRandomized) {
                            ...
                            $randomize_button_onclick = "randomizeDialog('".js_escape(strip_tags(label_decode($_GET['id'])))."'); return false;";

                            $randomize_button = "<button id='redcapRandomizeBtn' class='jqbuttonmed' onclick=\"$randomize_button_onclick\" $disableRandBtn><span style='vertical-align:middle;color:green;'><i class=\"fas fa-random\"></i> ".RCView::tt("random_51")."</span></button>";
                        }
                        ...
                        $(function(){
                            ...
                            if (randomizationFieldTdObj.length) {
                                ...
                                randomizationFieldTdObj.html('<?=js_escape($randomize_button)?><div id="randomizationFieldHtml" style="<?=$randomizeFieldDisplay?>">'+randomizationFieldHtml+'</div>');
                            } else {
                                // Left-aligned
                                ...
                                var randomizationFieldHtml = randomizationFieldTdObj.html();
                                randomizationFieldTdObj.html('<div class="randomizationDuplLabel">'+labelText+'<div class="space"></div></div><?=js_escape($randomize_button)?><div id="randomizationFieldHtml" style="<?=$randomizeFieldDisplay?>">'+randomizationFieldHtml+'</div>');
            ...
            elseif ($isSurveyPage && isset($randomizationEnabled) && $randomizationEnabled && isset($wasRecordRandomized) && $wasRecordRandomized
                    && isset($randomizationCriteriaFields[$name]) && $randomizationCriteriaFields[$name] == $_GET['event_id'])
            {
            ...
            if ($rr_type == 'matrix_header' || (isset($rr_array['field']) && $rr_type != 'hidden'))
            {
                ...
                if ($defaultValue != '' && $pageHasData && isset($value) && $value != "") {
                ...
                if ($isSurveyPage)
                {
                    ...
                    if (isset($question_auto_numbering) && !$question_auto_numbering && $rr_type != 'matrix_header' && isset($Proj->metadata[$rr_array['field']])) {
            ...
            if (PAGE == "Design/online_designer.php" || PAGE == "Design/online_designer_render_fields.php")
            {
                ...
                if ((isset($rr_array['matrix_field']) && $rr_array['matrix_field'] == '1' && $rr_type == "header") || (isset($rr_array['matrix_field']) && $rr_array['matrix_field'] == '1' && !isset($rr_array['hasSH']))) {
                    $matrixGroupIcons = 
                        "<div class='design-matrix-icons' data-matrix-group-name='{$rr_array['grid_name']}'>
                            <a href='javascript:;' onclick=\"openAddMatrix('{name}','')\" ignore='Yes' title='".RCView::tt_attr("design_1151")."' data-bs-toggle='tooltip' class='field-action-link' data-field-action='edit-matrix'>".RCIcon::OnlineDesignerEdit()."</a>
                            <a href='javascript:;' onclick=\"copyMatrix('{$rr_array['grid_name']}')\" ignore='Yes' title='".RCView::tt_attr("design_1220")."' data-bs-toggle='tooltip' class='field-action-link' data-field-action='copy-matrix'>".RCIcon::OnlineDesignerCopy()."</a>
                            <a href='javascript:;' onclick=\"moveField('','{$rr_array['grid_name']}')\" ignore='Yes' title='".RCView::tt_attr("design_1152")."' data-bs-toggle='tooltip' class='field-action-link' data-field-action='move-matrix' draggable='matrix'>".RCIcon::OnlineDesignerMove()."</a>
                            <a href='javascript:;' onclick=\"deleteMatrix('{name}','{$rr_array['grid_name']}');\" ignore='Yes' title='".RCView::tt_attr("design_1153")."' data-bs-toggle='tooltip' class='field-action-link me-2' data-field-action='delete-matrix'>".RCIcon::OnlineDesignerDelete()."</a>
                ...
                if ($rr_type == "header") {
                    ...
                    $contains_par_info = 0;
                    $miscField = $Proj->metadata[$rr_array['field']]['misc'] ?? '';
                    if ($mycap_enabled &&
                    ...
                    $this_bookend1 = str_replace(array("{field_icons}","{matrixHdrs}","{addFieldBtnStyle}","{matrixGroupIcons}","{matrixGroupName}","{style-display}"),
                                                 array($fieldIcons, $matrixHdrsRepl,$addFieldBtnStyle,$matrixGroupIcons,$rr_array["grid_name"],""),
                                                 $bookend1);
                }
                ...
                $multi_field_checkbox = "<input type='checkbox' class='float-end qef-select-checkbox' id='mfsckb-design-{$name}' ignore='Yes'>";
                ...
                if ($surveys_enabled) {
                    ...
                    $this_form = $this_ProjFields[$name]["form_name"];
                ...
                $repl1 = array(
                    ...
                    $rr_type["grid_name"] ?? "",
                    // {fieldHasMisc}
                    empty($GLOBALS['status'] < 1 ? $Proj->metadata[$name]['misc'] : $Proj->metadata_temp[$name]['misc']) ? "0" : "1",
                    // {fieldHasBL}
                    empty($GLOBALS['status'] < 1 ? $Proj->metadata[$name]['branching_logic'] : $Proj->metadata_temp[$name]['branching_logic']) ? "0" : "1",
                ...
                if (isset($rr_array['action_tag_class_design'])) {
                    $this_bookend2 = str_replace("{action_tags}", '<tr><td class="frmedit actiontags" colspan="2"><div><code>'.htmlspecialchars($rr_array['action_tag_class_design'], ENT_QUOTES).'</code></div></td></tr>', $bookend2);
                } else {
                ...
                && !in_array($rr_type, array("static", "hidden", "button", "lock_record", "esignature", "descriptive", "matrix_header")))
            {
                ...
                if ($data_resolution_enabled > 0) {
                    ...
                    if (isset($name) && isset($Proj->metadata[$name]['form_name']))
                    {
                        ...
                        $hasFormEditRights = (isset($user_rights['forms'][$Proj->metadata[$name]['form_name']]) && $user_rights['forms'][$Proj->metadata[$name]['form_name']] != '2');
                    }
                ...
                if (!empty($missingDataCodes) && !$disable_all && !in_array('@NOMISSING', $rr_array['action_tags']) && !$hasReadonlyActionTag
                    ...
                    && !($randomizationEnabled && $randomizationField == $name && $randomizationEvent == $_GET['event_id'])
                ) {
            ...
            switch ($rr_type)
            {
                ...
                case 'descriptive':
                    ...
                    if (isinteger($edoc_id))
                    {
                        ...
                        else
                        {
                            ...
                            elseif ($edoc_display_img == '1' && in_array(strtolower($edoc_info['file_extension']), $img_types))
                            {
                                ...
                                // Inline image
                                print "<br><br><img lsrc='$image_view_page&doc_id_hash=".Files::docIdHash($edoc_id)."&id=$edoc_id&instance={$_GET['instance']}".($isSurveyPage ? "&s=".$_GET['s'] : "")."' "
                    ...
                    elseif ($enable_field_attachment_video_url && $video_url != '')
                    {
                        ...
                        // Display inside popup
                            ...
                            print  "<div $video_data_mlm class='div_embed_video'>
                                        <button ignore='Yes' class='btn btn-xs btn-defaultrc fs13' onclick=\"openEmbedVideoDlg('".js_escape($video_url_formatted)."',$unknown_video_service,'$name','".htmlspecialchars($video_custom_html,ENT_QUOTES)."');return false;\">
                ...
                case 'lock_record':
                    ...
                    print  "><label for='__LOCKRECORD__' style='color:#A86700;cursor:pointer;' class='font-weight-bold ms-2'><img src='".APP_PATH_IMAGES."lock.png' style='position:relative;top:-2px;'> ".RCView::tt("form_renderer_18")."</label>";
                ...
                case 'checkbox':
                    ...
                    $maxChecked = (int)(($isFormOrSurveyPage && in_array('@MAXCHECKED', $rr_array['action_tags'])) ? Form::getValueInActionTag($Proj->metadata[$name]['misc'], "@MAXCHECKED") : 0);
                ...
                case 'hidden':
                    ...
                    $fv = "";
                    if (isset($Proj->metadata[$name]) && $Proj->metadata[$name]['element_validation_type'] !== null && $Proj->metadata[$name]['element_type'] == 'text' && $name != $Proj->table_pk && !isset($missingDataCodes[$value]))
                    {
                        $fv = "fv='".$Proj->metadata[$name]['element_validation_type']."'";
                    ...
                    if ($name == $Proj->table_pk && Form::hasHiddenPdfActionTag($Proj->metadata[$name]['misc'], $Proj->project_id, $_GET['id'], $_GET['event_id'], $_GET['page'], $_GET['instance'])) {
                ...
                case 'textarea':
                    ...
                    if(isset($missingDataCodes[$value]) && !Form::hasActionTag("@NOMISSING", $Proj->metadata[$name]['misc'])){
                        $disabled='disabled';
                    }
                    // Output row
                ...
                case 'select':
                    ...
                    if (PAGE != 'install.php' && isset($Proj->metadata[$name]) && $Proj->metadata[$name]['element_type'] != 'sql' && isset($value) && !is_numeric($value)
                    ...
                    if (isset($Proj->metadata[$name]) && $Proj->metadata[$name]['element_type'] == 'sql') {
                    ...
                    if ((isset($_GET['page']) && $name==$_GET['page']."_complete") || $isSurveyPage || in_array('@NOMISSING', $rr_array['action_tags'])){
                        $ddlOptions=$rr_array['enum'];
                    }else{
                        $ddlOptions=$rr_array['enum']."\n".$missingDataEnum;
                    }
                    // Render the drop-down
                    list ($ddOptionHtml, $ddDisabled) = DataEntry::render_dropdown($ddlOptions, $value, "", $name, $hasRandomOrderActionTag, $hideChoices, $maxChoices, (isset($_GET['page']) && $_GET['page']."_complete" == $name), $enable_dd_auto_complete);
                    ...
                    print "<span data-kind='field-value'>";
                    print  "<span $class><select {$aria_required} aria-labelledby='$ariaLabelledBy' class='x-form-text x-form-field $dd_auto_complete_class $randFldClass $opacityClass' name='$name' $id $disabled $onchange $onclick $onblur $onfocus $tabindex>";
                ...
                case 'text':
                    ...
                    if(isset($missingDataCodes[$value]) && !Form::hasActionTag("@NOMISSING", $Proj->metadata[$name]['misc'])){
                        ...
                        ?><script type="text/javascript">$(function(){ $('#<?=$name?>-tr button:not(.rc-autocomplete), #<?=$name?>-tr .ui-datepicker-trigger').hide(); });</script><?php
                    }
                    ...
                    if (isset($rr_array['validation']))
                    {
                        ...
                        if ($rr_array['validation'] == 'date' || $rr_array['validation'] == 'date_ymd' || $rr_array['validation'] == 'date_mdy' || $rr_array['validation'] == 'date_dmy')
                        {
                            if (!$disable_all) {
                                ...
                                if ($display_today_now_button) $nowBtn = "<button ignore='Yes' class='jqbuttonsm ms-2 today-now-btn d-print-none' onclick=\"setToday('$name','{$rr_array['validation']}');return false;\">".RCView::tt("dashboard_32")."</button>";
                                $nowBtn .= $dformat;
                            } else {
                        ...
                        elseif ($rr_array['validation'] == 'time')
                        {
                            if (!$disable_all) {
                                $newclass = "time2";
                                if ($display_today_now_button) $nowBtn = "<button ignore='Yes' class='jqbuttonsm ms-2 today-now-btn d-print-none' onclick=\"setNowTime('$name');return false;\">".RCView::tt("form_renderer_29")."</button>";
                                $dformat = MetaData::getDateFormatDisplay($rr_array['validation']);
                                $nowBtn .= $dformat;
                            } else {
                        ...
                        elseif ($rr_array['validation'] == 'time_hh_mm_ss')
                        {
                            if (!$disable_all) {
                                $newclass = "time3";
                                if ($display_today_now_button) $nowBtn = "<button ignore='Yes' class='jqbuttonsm ms-2 today-now-btn d-print-none' onclick=\"setNowTime('$name',true);return false;\">".RCView::tt("form_renderer_29")."</button>";
                                $dformat = MetaData::getDateFormatDisplay($rr_array['validation']);
                                $nowBtn .= $dformat;
                            } else {
                        ...
                        elseif ($rr_array['validation'] == 'datetime' || $rr_array['validation'] == 'datetime_ymd' || $rr_array['validation'] == 'datetime_mdy' || $rr_array['validation'] == 'datetime_dmy')
                        {
                            if (!$disable_all) {
                                ...
                                $dformat = MetaData::getDateFormatDisplay($rr_array['validation']);
                                if ($display_today_now_button) $nowBtn = "<button ignore='Yes' class='jqbuttonsm ms-2 today-now-btn d-print-none' onclick=\"setNowDateTime('$name',0,'{$rr_array['validation']}');return false;\">".RCView::tt("form_renderer_29")."</button>";
                                $nowBtn .= $dformat;
                            } else {
                        ...
                        elseif ($rr_array['validation'] == 'datetime_seconds' || $rr_array['validation'] == 'datetime_seconds_ymd' || $rr_array['validation'] == 'datetime_seconds_mdy' || $rr_array['validation'] == 'datetime_seconds_dmy')
                        {
                            if (!$disable_all) {
                                ...
                                $dformat = MetaData::getDateFormatDisplay($rr_array['validation']);
                                if ($display_today_now_button) $nowBtn = "<button ignore='Yes' class='jqbuttonsm ms-2 today-now-btn d-print-none' onclick=\"setNowDateTime('$name',1,'{$rr_array['validation']}');return false;\">".RCView::tt("form_renderer_29")."</button>";
                                $nowBtn .= $dformat;
                            } else {
                    ...
                    if (isset($cp->_fields_utilized[$name]) || ( (PAGE == "Design/online_designer.php" || PAGE == "Design/online_designer_render_fields.php")
                        ...
                        $viewEqLink = "&nbsp;&nbsp;<a href='javascript:;' ignore='Yes' class='viewEq d-print-none' tabindex='-1' onclick=\"viewEq('$name',$calcDateParam,$calcTextParam);\">".RCView::tt("form_renderer_21")."</a>";
                    }
                    ...
                    if (in_array("@CONSENT-VERSION", $rr_array['action_tags']) && $value == '' && $isSurveyPage) {
                        ...
                        $nowBtn .= "&nbsp;&nbsp;<a ignore='Yes' class='smalllink' style='color:#800000;font-family:Tahoma;' href='javascript:;' onclick=\"simpleDialog(".($isSurveyPage ? "window.lang.data_entry_262" : "window.lang.data_entry_263").",window.lang.form_renderer_22);\">".RCview::tt("form_renderer_22")."</a>";
                    } elseif (in_array("@LATITUDE", $rr_array['action_tags'])) {
                        $style = "style='max-width:130px;'";
                        $nowBtn = "&nbsp;&nbsp;<button ignore='Yes' class='jqbuttonsm' onclick=\"getGeolocation('latitude','$name','form',true);dataEntryFormValuesChanged=true;return false;\">".RCView::tt("global_125")."</button>";
                        $nowBtn .= "&nbsp;&nbsp;<a data-kind='reset-link' ignore='Yes' class='smalllink' href='javascript:;' onclick=\"$('[name=$name]').val('');dataEntryFormValuesChanged=true;try{calculate('$name');doBranching('$name');}catch(e){} return false;\">".RCView::tt("form_renderer_20")."</a>";
                    } elseif (in_array("@LONGITUDE", $rr_array['action_tags'])) {
                        $style = "style='max-width:130px;'";
                        $nowBtn = "&nbsp;&nbsp;<button ignore='Yes' class='jqbuttonsm' onclick=\"getGeolocation('longitude','$name','form',true);dataEntryFormValuesChanged=true;return false;\">".RCView::tt("global_125")."</button>";
                        $nowBtn .= "&nbsp;&nbsp;<a data-kind='reset-link' ignore='Yes' class='smalllink' href='javascript:;' onclick=\"$('[name=$name]').val('');dataEntryFormValuesChanged=true;try{calculate('$name');doBranching('$name');}catch(e){} return false;\">".RCView::tt("form_renderer_20")."</a>";
                    }
                ...
                case 'calc':
                    ...
                    print  "<span data-kind='field-value'><input type='text' aria-labelledby='$ariaLabelledBy' name='$name' value='".htmlspecialchars($value??"", ENT_QUOTES)."' sv='".htmlspecialchars($value??"", ENT_QUOTES)."' $id $onfocus tabindex='-1' readonly='readonly'
                                class='x-form-text x-form-field rci-calc'></span>&nbsp;&nbsp;<a href='javascript:;' ignore='Yes' class='viewEq d-print-none' tabindex='-1' onclick=\"viewEq('$name',0,0);\">".RCView::tt("form_renderer_21")."</a>
                                $note
                            </td>";
                    break;
                ...
                case 'file':
                    ...
                    $inlineImageDimensionsAttr = self::parseInlineActionTagAttr($Proj->metadata[$name]['misc']);
                    ...
                    if ($edoc_field_option_enabled) {
                        ...
                        if (!isset($value) || $value == '' || $missing) {
                            ...
                            if ((!isset($user_rights) || (is_array($user_rights) && empty($user_rights)) || (isset($user_rights) && ($user_rights['forms'][$_GET['page']] == '1' || $user_rights['forms'][$_GET['page']] == '3'))) && !$disable_all) {
                            ...
                            $q_fileup_query = db_query("select doc_name, doc_size from redcap_edocs_metadata where doc_id = $value and project_id = $project_id limit 1");
        ...
        // PDF and image file preview enhancements (not in Online Designer)
                             ...
                             $maxChoices=array(), $isFormStatus=false, $enable_dd_auto_complete=false)
    {
        ...
        $hasMissingDataCodes = (!$isFormStatus && !empty($missingDataCodes) && isset($Proj->metadata[$name]) && !Form::hasActionTag("@NOMISSING", $Proj->metadata[$name]['misc']));
        $isSqlField = (isset($Proj->metadata[$name]) && $Proj->metadata[$name]["element_type"] == "sql");
                          ...
                          $hideChoices=array(), $ariaLabelledBy="", $maxChoices=array())
    {
        ...
        //missing data stuff
        $missing = (isset($missingDataCodes[$element_value]) && isset($Proj->metadata[$name]) && !Form::hasActionTag("@NOMISSING", $Proj->metadata[$name]['misc']));
            ...
            foreach ($select_choices_array as $this_value=>$this_text)
            {
                ...
                if ($isMatrixField) {
                    ...
                    if ($enhanced_choices) {
                        ...
                        $enhanced_choices_class2 = ($readonly || $maxChoiceReached) ? '' : 'hover';
                               ...
                               $hideChoices=array(), $ariaLabelledBy="", $maxChoices=array(), $maxChecked=0)
    {
        ...
        if (isset($missingDataCodes[$MDValue]) && isset($Proj->metadata[$name]) && !Form::hasActionTag("@NOMISSING", $Proj->metadata[$name]['misc'])) {
            $dataMissing=true;
            $disabled="disabled";
        }
        // Set parameters
    ...
    public static function addEmptyRequiredFieldsToPost()
    {
        ...
        if (isset($_POST['empty-required-field']) && is_array($_POST['empty-required-field'])) {
            ...
            foreach ($emptyReqFields as $key=>$emptyReqField) {
                ...
                if (!isset($Proj->metadata[$emptyReqField])) {
                ...
                if (!isset($_POST[$emptyReqField])) {
                    if ($Proj->isCheckbox($emptyReqField)) {
                        foreach (array_keys(parseEnum($Proj->metadata[$emptyReqField]['element_enum'])) as $emptyReqFieldChoice) {
                                      ...
                                      $response_id=null, $callSaveRecordHook=false, $surveyJustCompleted=false)
    {
        ...
        if ($enable_edit_survey_response && $recordExists && $survey_id != null && PAGE == 'DataEntry/index.php' && $user_rights['forms'][$_GET['page']] != '3')
        {
                ...
                if (strpos($field_name, "-") === false && $field_name != 'hidden_edit_flag' && $field_name != '__old_id__' && !(substr($field_name,0,10) == '_checkbox_' && $value == 'on'))
                {
                    ...
                    if (isset($current_data[$field_name]) && !$is_checkbox) {
                        if ($value !== $current_data[$field_name]) {
                            ...
                            if ($value == '' && $Proj->metadata[$field_name]['element_type'] == 'file')
                            {
        ...
        $rss->checkLogicBasedTrigger(PROJECT_ID, $fetched, $_GET['page'], $_GET['event_id'], $_GET['instance']);
    ...
    public static function changeRecordId($old_id, $new_id, $project_id = null)
    {
        ...
        db_query($sql);
        // Change record id in redcap_ehr_import_counts
        $sql_all[] = $sql = "UPDATE redcap_ehr_import_counts SET record = '" . db_escape($new_id) . "'
                             WHERE project_id = " . $project_id . " AND record = '" . db_escape($old_id) . "'";
        db_query($sql);
    ...
    public static function render_context_msg($custom_record_label, $custom_event_label, $context_msg)
    {
        ...
        if ($Proj->longitudinal)
        {
            ...
            $instanceNum = ($is_repeating_event && $_GET['instance'] > 1) ? ("<span style='margin-left:4px;'>".RCView::tt_i("data_entry_541", array($_GET['instance']))."</span>") : "";
...
//                        order by rl.record_int desc limit 1";
    ...
    public static function getCalcFields($form)
    {
        ...
        $calc_triggers = array();
        $fieldsThisForm = isset($Proj->forms[$form]) ? array_keys($Proj->forms[$form]['fields']) : array();
        ...
        if (!empty($subquery_array)) {
            $pre_sql = "select field_name from redcap_metadata where element_type = 'calc' and form_name != '$form'
        ...
        $sql = "select field_name, element_enum, form_name, element_type, misc from redcap_metadata 
                where ((element_type = 'calc' and element_enum != '') or (element_type = 'text' and misc like '%@CALCDATE%') or (element_type = 'text' and misc like '%@CALCTEXT%'))
    ...
    {
        global $lang, $user_rights, $table_pk, $cp, $bl, $longitudinal, $auto_inc_set, $repeatingFieldsEventInfo;
        ...
        foreach ($calc_triggers as $this_field=>$this_enum)
        {
            ...
            // Add field
            $cp->feedEquation($this_field, $this_enum, $Proj->metadata[$this_field]);
        }
        ...
        if (isset($Proj->forms[$form_name])) {
            foreach (array_keys($Proj->forms[$form_name]['fields']) as $field_name)
            {
                ...
                if ($field_req == '1' && $element_type != 'descriptive')
                {
                    ...
                    $fieldReqText = (PAGE == "surveys/index.php" && $Proj->surveys[$Proj->forms[$_GET['page']]['survey_id']]['show_required_field_text'] == '2') ? "" : " ".RCView::tt("data_entry_39");
                ...
                if ($Proj->metadata[$field_name]['misc'] !== null && strpos($Proj->metadata[$field_name]['misc'], '@') !== false) {
                ...
                if ($element_type == 'slider')
                {
                    ...
                    $form_element["slider_min"] = (is_numeric($Proj->metadata[$field_name]['element_validation_min']) ? $Proj->metadata[$field_name]['element_validation_min'] : 0);
                    $form_element["slider_max"] = (is_numeric($Proj->metadata[$field_name]['element_validation_max']) ? $Proj->metadata[$field_name]['element_validation_max'] : 100);
                }
                ...
                if ($element_type == 'text' || $element_type == 'calc')
                {
                        ...
                        && !($field_name == $table_pk && $removeRecordIdValidation))
                    {
                        ...
                        if (isset($Proj->metadata[$field_name]['misc']) && strpos($Proj->metadata[$field_name]['misc'], '@FORCE-MINMAX') !== false) {
                ...
                if ($useCalc || $useBranch)
                {
                    ...
                    if ($element_type == 'radio' || $element_type == 'yesno' || $element_type == 'truefalse') {
                        ...
                        $js = (!$grid_rank) ? "" :	"matrix_rank(this.value,'$field_name','" . implode(",", (isset($Proj->matrixGroupNames[$grid_name]) && is_array($Proj->matrixGroupNames[$grid_name]) ? $Proj->matrixGroupNames[$grid_name] : [])) . "');";
    ...
    public static function checkReqFields($fetched, $isSurveyPage=false, $reqmsg_maxlength = 1500)
    {
        ...
        if (isset($_POST['submit-action']) && !in_array($_POST['submit-action'], $ignoreSubmitActions))
        {
            ...
            if ($isSurveyPage && isset($_POST['__page__']) && isset($pageFields[$_POST['__page__']])) {
                // In case this is a multi-page survey, only use the fields on this survey page
                $thisPageFields = [];
                foreach ($pageFields[$_POST['__page__']] as $this_field) {
                    $thisPageFields[$this_field] = label_decode($Proj->metadata[$this_field]['element_label']);
                }
            } else {
                // Default: Use all fields on this instrument
                $thisPageFields = $Proj->forms[$_GET['page']]['fields'];
            }
            ...
            foreach ($thisPageFields as $this_field=>$this_label)
            {
                ...
                if ($Proj->metadata[$this_field]['field_req'] && !in_array($this_field, $emptyReqFields))
                {
                    ...
                    if (($isSurveyPage && Form::hasHiddenOrHiddenSurveyActionTag($Proj->metadata[$this_field]['misc'], $Proj->project_id, $fetched, $_GET['event_id'], $_GET['page'], $_GET['instance']))
                        || (!$isSurveyPage && Form::hasHiddenOrHiddenFormActionTag($Proj->metadata[$this_field]['misc'], $Proj->project_id, $fetched, $_GET['event_id'], $_GET['page'], $_GET['instance']))) {
                        continue;
                    }
                    // Set flag
                    ...
                    elseif ($Proj->isCheckbox($this_field) && !isset($_POST["__chkn__".$this_field]))
                    {
                        ...
                        $doReqChk = false;
                        $thisCheckboxEnum = parseEnum($Proj->metadata[$this_field]['element_enum']);
    ...
    public static function checkSimultaneousUsers()
    {
        ...
        if (isset($_GET['page']) && isset($user_rights['forms'][$_GET['page']]) && $user_rights['forms'][$_GET['page']] == '2') return false;
        ...
        <?php if (Files::fileUploadPasswordVerifyExternalStorageEnabledProject(PROJECT_ID)) { ?>
            <div id="file_upload_vault_popup" title="<?=RCView::tt_js2($isSurveyPage ? "data_entry_455" : "data_entry_453")?>" class="simpleDialog fs14" style="line-height:1.5;background-color:#e1eee1;">
                ...
                <?php if (!$isSurveyPage) { ?>
                    ...
                    <div class="clearfix my-2">
                        <div style="float:left;display:block;margin-left:50px;width:130px;font-weight:bold;"><?=($GLOBALS['two_factor_auth_enabled'] && $GLOBALS['two_factor_auth_esign_pin'] ? $lang['global_252'] : $lang['global_240'])?></div>
                        <div style="float:left;display:block;">
                            <input type="password" id="file_upload_vault_password" autocomplete="new-password" class="x-form-text x-form-field">
                        </div>
                    </div>
                <?php } ?>
                        ...
                        <i class="fas fa-info-circle"></i> <?=RCView::tt("data_entry_577")?>
                         <?php if ($GLOBALS['two_factor_auth_twilio_enabled'] || $GLOBALS['two_factor_auth_email_enabled']) { ?>
                            ...
                            <?php if ($GLOBALS['two_factor_auth_email_enabled']) { ?>
                                <a href="javascript:;" class="fs14 text-decoration-underline ms-1" onclick="sendTFAcode('email',true);$('#sent-msg-tfa-code').removeClass('hide');setTimeout(function(){ $('#sent-msg-tfa-code').addClass('hide'); },2000);"><?=RCView::tt("data_entry_579")?></a>
                            <?php } ?>
                            <?php if ($GLOBALS['two_factor_auth_twilio_enabled']) { ?>
                                <a href="javascript:;" class="fs14 text-decoration-underline ms-1" onclick="sendTFAcode('sms',true);$('#sent-msg-tfa-code').removeClass('hide');setTimeout(function(){ $('#sent-msg-tfa-code').addClass('hide'); },2000);"><?=RCView::tt("data_entry_580")?></a>
                            <?php } ?>
            ...
            foreach ($attr1 as $attr2) {
                foreach ($attr2 as $attr3) {
                    foreach (array_keys($attr3) as $field) {
                        ...
                        $fields[] = $Proj->metadata[$field]['form_name']."_complete";
                    }
                }
            }
        }
                ...
                foreach ($attr2 as $instance=>$attr3) {
                    foreach (array_keys($attr3) as $field) {
                        ...
                        if ($valueIsArray || $Proj->isCheckbox($field)) {
                            ...
                            if (!$valueIsArray) {
                                $value = array_fill_keys(array_keys(parseEnum($Proj->metadata[$field]['element_enum'])), '0');
                            }
                            // Checkbox values
                            ...
                            {
                                $fv = "fv=\"".$Proj->metadata[$field]['element_validation_type']."\"";
                                if (substr($Proj->metadata[$field]['element_validation_type'], -4) == '_mdy') {
                                    list ($this_date, $this_time) = explode(" ", $value);
                                    $value = trim(DateTimeRC::date_ymd2mdy($this_date) . " " . $this_time);
                                } elseif (substr($Proj->metadata[$field]['element_validation_type'], -4) == '_dmy') {
                                    list ($this_date, $this_time) = explode(" ", $value);
                                    $value = trim(DateTimeRC::date_ymd2dmy($this_date) . " " . $this_time);
                                }
                            }
    ...
    public static function renderSearchUtility()
    {
        ...
        foreach ($Proj->metadata as $row)
        {
            ...
            if ($user_rights['forms'][$row['form_name']] == '0' && $row['field_name'] != $Proj->table_pk) continue;
            // Build list option
	...
	{
		global $Proj;
		if (!isset($Proj->matrixGroupNames[$matrix_group_name])) return false;
		$action_tags_regex = $last_hide_choice_val = null;
		foreach ($Proj->matrixGroupNames[$matrix_group_name] as $field)
		{
    ...
    {
        global $missingDataCodes, $Proj;
        if (isset($missingDataCodes[$code]) && !Form::hasActionTag("@NOMISSING", $Proj->metadata[$name]['misc'])) {
    ...
    public static function getRecordCustomEventLabel($record, $this_event_id, $this_instance)
    {
        global $Proj;
        $custom_event_labels_all = "";
        foreach ($Proj->eventInfo as $attr) {
        ...
        $event_label_piping_data = Records::getData('array', $record, $custom_event_label_fields);
        return Piping::replaceVariablesInLabel($Proj->eventInfo[$this_event_id]['custom_event_label'], $record, $this_event_id, $this_instance, $event_label_piping_data, false, null, false);
    }
