...
class PDF
{
	...
	public static function output($survey_mode = false, $hideAllHiddenAndHiddenSurveyActionTagFields = false, $selected_forms_events_array = array(), $returnString=false)
	{
		...
		if (isset($_GET['s']) && !empty($_GET['s']))
		{
            ...
            $sql = "select 1 from redcap_surveys_participants p, redcap_surveys_response r
                    ...
                    and p.survey_id = '" . db_escape($Proj->forms[$_GET['page']]['survey_id']) . "' and p.event_id = '" . db_escape($_GET['event_id']) . "' 
		...
		if (isset($_GET['id']) && !isset($_GET['page']))
		{
            ...
            if (is_array(PDF::$selected_forms_events_array) && !empty(PDF::$selected_forms_events_array)) {
                ...
                foreach (PDF::$selected_forms_events_array as $this_event_id=>$these_forms) {
                    ...
                    foreach ($these_forms as $this_form) {
                        $getDataFields = array_merge($getDataFields, array_keys($Proj->forms[$this_form]['fields']));
                    }
                }
            } else {
			...
			$params = array('records'=>$_GET['id'], 'events'=>$getDataEvents, 'fields'=>$getDataFields, 'groups'=>$user_rights['group_id'], 'removeMissingDataCodes'=>true);
            ...
            elseif (isset($_GET['id']) && isset($_GET['event_id']) && isset($_GET['instance']) && isset(PDF::$selected_forms_events_array[$_GET['event_id']])
                ...
                if (isset($Proj->forms[$currentFormName]) && $Proj->isRepeatingFormOrEvent($_GET['event_id'], $currentFormName))
                {
		...
		$context_builder = Context::Builder()
			...
			->page_fields(array_keys($Proj->metadata)) // all fields
		...
		if (!isset($data['']) && !empty($data))
		{
			...
			$deidFieldsToRemove = ($user_rights['data_export_tool'] > 0)
		...
		// Render the PDF
        if ($returnString) {
            return PDF::renderPDF($metadata, $acknowledgement, $project_name, $data, isset($_GET['compact']), $pdf_translations, ($deidFieldsToRemove ?? null), $returnString);
        } else {
            PDF::renderPDF($metadata, $acknowledgement, $project_name, $data, isset($_GET['compact']), $pdf_translations, ($deidFieldsToRemove ?? null), $returnString);
        }
	}
	...
	public static function new_page_check($num_lines, $pdf, $study_id_event, $forceNewPage=false)
	{
		if ($forceNewPage || ((self::y_units_per_line * $num_lines) + $pdf->GetY() > self::bottom_of_page))
        {
            ...
            // Add page number
			$pdf->Cell(0, 5, RCView::interpolateLanguageString(self::$page_num_template, [$pdf->PageNo()]), 0, 1, 'R');
	 ...
	 */
	public static function renderPDF($metadata, $acknowledgement, $project_name = "", $Data = array(), $compactDisplay=false, $translations = array(), $deid_fields = array(), $returnString = false)
	{
				...
				foreach ($eventNormalized as $event_id=>$nattr1)
				{
					...
					foreach ($nattr1 as $repeat_instrument=>$nattr2)
					{
						foreach ($nattr2 as $repeat_instance=>$dataNormalized)
						{
                            ...
                            $this_metadata = self::pipeMetadata($this_metadata, $Proj->project_id, $record, $event_id, $row["form_name"], $repeat_instance, $piping_record_data, $pdfHasData);
								...
								// foreach ($this_metadata as $row)
							{
								...
								if ($last_form != $row['form_name']) // || $last_repeat_instrument != $repeat_instrument || $last_repeat_instance != $repeat_instance)
								{
                                    ...
                                    if ($GLOBALS['orig_lang_id'] != $lang_id) {
                                        ...
                                        $pdf_translations = MultiLanguage::translatePDF($GLOBALS['context'], $metadata, $acknowledgement, $project_name, $Data);
                                        $this_metadata = self::pipeMetadata($metadata, $Proj->project_id, $record, $event_id, $row["form_name"], $repeat_instance, $piping_record_data, $pdfHasData);
                                    ...
                                    if (isset($Proj->forms[$row["form_name"]]["survey_id"]) && Econsent::econsentEnabledForSurvey($Proj->forms[$row["form_name"]]['survey_id']))
                                    {
                                        // Record in a DAG?
                                        $dag_id = ($record == null) ? null : Records::getRecordGroupId($Proj->project_id, $record);
                                        // Get the e-Consent settings for this instrument
                                        $eConsentSettings = Econsent::getEconsentSurveySettings($Proj->forms[$row["form_name"]]["survey_id"], $dag_id, $lang_id);
                                        ...
                                        if (Survey::isResponseCompleted($Proj->forms[$row["form_name"]]["survey_id"], $record, $event_id, ($repeat_instance == '0' ? '1' : $repeat_instance))) // Survey has been completed
                                        {
                                            ...
                                            $sql = "select f.consent_form_filter_lang_id 
                                            ...
                                            $params = [$eConsentSettings['consent_id'], $record, $event_id, $Proj->forms[$row["form_name"]]["survey_id"], ($repeat_instance == '0' ? '1' : $repeat_instance)];
                                            $q = db_query($sql, $params);
                                            if (db_num_rows($q)) {
                                                $temp_lang_id = db_result($q);
                                        ...
                                        if (isset($_GET['appendEconsentFooter']))
                                        {
                                            list ($nameDobText, $versionText, $typeText) = Econsent::getEconsentOptionsData($Proj->project_id, $_GET['id'], $row["form_name"], $dag_id, $lang_id);
                                            ...
                                            if ($typeText != '') {
                                                $versionTypeTextArray[] = (isset($translations[$row["form_name"]]) ? $translations[$row["form_name"]]["data_entry_429"] : RCView::getLangStringByKey("data_entry_429"))." ".$typeText; //= Type:
                                        ...
                                        $ec = new Econsent();
                                        $consent_id = isset($Proj->forms[$row["form_name"]]['survey_id']) ? ($ec->getEconsentBySurveyId($Proj->forms[$row["form_name"]]['survey_id'])['consent_id'] ?? null) : null;
                                        $_GET["appendToHeaderEconsent"] = ($consent_id == null) ? "" : $ec->getCustomEconsentLabel($Proj->project_id, $record, $event_id, $row["form_name"], $repeat_instance, $consent_id);
                                    }
									...
									if (isset($Proj) && is_array($ProjForms) && isset($ProjForms[$row['form_name']]['survey_id'])) {
										...
										$survey_instructions = str_replace(array("\r", "\n", ), array("", ""), PDF::formatText(label_decode(label_decode($Proj->surveys[$survey_id]['instructions']))));
										$survey_instructions = str_replace("\t", " ", label_decode(Piping::replaceVariablesInLabel($survey_instructions, $record, $event_id, $repeat_instance, $piping_record_data, true, null, false, "", 1, false, true, $row['form_name'], null, false, false, false, true)));
									...
									//Initial page number
									$pdf->Cell(0, 2, RCView::interpolateLanguageString(self::$page_num_template, [$pdf->PageNo()]), 0, 1, 'R');
									...
									{
										$pdf->MultiCell(0,self::row_height,$survey_instructions,0);
										$pdf->Ln();
								...
								if ($row['element_type'] == "yesno" || $row['element_type'] == "truefalse" || $row['element_type'] == "radio" || $row['element_type'] == "select" || $row['element_type'] == "advcheckbox" || $row['element_type'] == "checkbox" || $row['element_type'] == "sql")
								{
                                    ...
                                    if (isset($econsentEnabledDescriptiveField) && $econsentEnabledDescriptiveField == $row['field_name'] && $econsentEnabledDescriptiveFieldDocId == null) {
                                        $this_string .= "\n\n".strip_tags(br2nl(PDF::formatText($econsentEnabledDescriptiveFieldLabel)));
                                    }
    ...
    public static function convertPdfToImages($pdf_doc_id, $project_id=null)
    {
        ...
        if ($pdf_file_path !== false) {
            try {
                ...
                for ($pdf_c = 0; $pdf_c < $imgCount; $pdf_c++) {
                    $this_file_path = APP_PATH_TEMP.$tmp_fname.($imgCount == 1 ? "" : '-'.$pdf_c).'.png';
                    ...
                    $this_page = $pdf_c+1;
                    $sql = "insert into redcap_pdf_image_cache (pdf_doc_id, page, image_doc_id, expiration) values (?, ?, ?, ?)";
                    $q = db_query($sql, [$pdf_doc_id, $this_page, $img_doc_id, $expiration]);
                }
                return $img_files;
            } catch (Throwable $e) { }
        }
    ...
    {
        $sql = "select image_doc_id from redcap_pdf_image_cache 
                where pdf_doc_id = ?
                order by page";
    ...
    // PIPING: If exporting a PDF with data, then any labels, notes, choices that have field_names in them for piping, replace out with data.
    public static function pipeMetadata($this_metadata, $project_id, $record, $event_id, $form_name, $repeat_instance, $piping_record_data, $pdfHasData)
    {
                ...
                if ($this_string != '')
                {
                    if ($record != '') {
                        ...
                        $this_string = Piping::replaceVariablesInLabel($this_string, $record, $event_id, $repeat_instance, $piping_record_data, true, null, false, "", 1,
                            false, true, $form_name, null, false, false, false, true);
