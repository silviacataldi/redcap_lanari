...
file_put_contents("C:\\my_pdfs\\all_records.pdf", $pdf_content);
	...
	public static function getPDF($record=null, $instrument=null, $event_id=null, $all_records=false, $repeat_instance=1, 
                                  ...
                                  $hideAllHiddenAndHiddenSurveyActionTagFields=false, $selected_forms_events_array=[])
	{
		...
		$pdfString = PDF::output($survey_mode, $hideAllHiddenAndHiddenSurveyActionTagFields, $selected_forms_events_array, true);
		// Reset $_GET params
		$_GET = $get_orig;
	...
	public static function setFormName($project_id, $form_name, $new_form_name)
	{
		...
		if ($status < 1)
		{
			...
			$sql = "update redcap_user_rights set
                    data_entry = replace(data_entry, '[".db_escape($form_name).",', '[$new_form_name,'),
                    data_export_instruments = replace(data_export_instruments, '[".db_escape($form_name).",', '[$new_form_name,')
			...
			$sql = "update $metadata_table set field_name = '{$new_form_name}_complete' where field_name = '".db_escape($form_name)."_complete' and project_id = $project_id";
			db_query($sql);
			// Change actual data table field_names to reflect the changed Form Status field
			$sql = "update ".\Records::getDataTable($project_id)." set field_name = '{$new_form_name}_complete' where field_name = '".db_escape($form_name)."_complete' and project_id = $project_id";
			db_query($sql);
			...
			$sql = "update redcap_form_display_logic_targets set form_name = '".db_escape($new_form_name)."' where form_name = '".db_escape($form_name)."'
					and event_id in ($eventIds)";
			db_query($sql);
            // Change back-end form name in mycap tasks table
            $sql = "UPDATE redcap_mycap_tasks SET form_name = '".db_escape($new_form_name)."' WHERE project_id = $project_id AND form_name = '".db_escape($form_name)."'";
            db_query($sql);
			// Change MultiLanguage metadata table
			MultiLanguage::updateFormNameDuringDevelopment($project_id, $form_name, $new_form_name);
		}
