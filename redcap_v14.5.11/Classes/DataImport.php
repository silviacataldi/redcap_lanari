...
class DataImport
{
	...
	public static function csvToArray($csv_filepath, $format='rows', $delimiter = ",")
	{
		...
		// Always from the BOM if the CSV file contains one
		$csvString1 = $csvString2 = file_get_contents($csv_filepath);
		$csvString1 = removeBOMfromUTF8($csvString1);
	...
	public static function storeAsyncDataRows($data, $delimiter, $date_format, $overwrite_behavior, $force_auto_number, $import_filename, $change_reason)
	{
		...
		fwrite($h, removeBOMfromUTF8($data));
		fseek($h, 0);
		...
		while (($row = fgetcsv($h, 0, $delimiter)) !== false) {
			...
			if (isset($recordsRowKeys[$record])) {
				...
				$sql = "update redcap_data_import_rows set row_data = concat(row_data, ?) where row_id = ?";
				$params = ["\n".$thisCsvRow, $row_id];
				...
				$params = [$import_id, $record, $event_id, $thisCsvRow];
				if (db_query($sql, $params)) {
	...
	public static function checkCompleteAsyncDataImport($import_id, $records_provided)
	{
		...
		if ($totalRecordsProcessed == $records_provided)
		{
			...
			if ($q && db_affected_rows() > 0)
			{
				...
				REDCap::email($row['user_email'], $GLOBALS['project_contact_email'], $subject, $message, '', '', $GLOBALS['project_contact_name'], [], $row['project_id']);
				return true;
			}
		}
		return false;
	...
	public static function getBatchSizeAsyncDataImport()
	{
		...
		if ($avg_time_per_record_ms != null) {
	...
	public static function processAsyncDataImports()
	{
        ...
        // If any individual batches have been stuck processing for more than 24 hours, set them as cancelled (since they are obviously never going to start)
        $oneDayAgo = date("Y-m-d H:i:s", mktime(date("H")-24,date("i"),date("s"),date("m"),date("d"),date("Y")));
        $sql = "select import_id from redcap_data_import where status = 'PROCESSING' and upload_time < '$oneDayAgo'";
        $q = db_query($sql);
        while ($row = db_fetch_assoc($q)) {
	...
	public static function loadBackgroundImportsTable()
	{
		...
		while ($row = db_fetch_assoc($q))
		{
			...
			$row['records_imported'] = ['sort'=>$row['records_imported'], 'display'=>number_format($row['records_imported'], 0, $formatDecimal, $formatThousands)];
            ...
            $errorDisplay = RCView::div(['class'=>($row['total_errors'] > 0 ? 'boldish' : '')], number_format($row['total_errors'], 0, $formatDecimal, $formatThousands));
	...
	public static function renderDataImportToolPage()
	{
		...
		renderPageTitle("<i class=\"fas fa-file-import\"></i> ".$lang['app_01']);
		...
		if (isset($_POST['async']) && $_POST['async'] == '1')
		{
            ...
            $change_reason = null;
            if ($require_change_reason && isset($_POST['change_reason'])) {
		...
		if (!isset($_REQUEST['submit']) && !isset($_POST['updaterecs']))
		{
			...
			if ($status < 1) {
				$devWarning =  "<div class='yellow' style='margin:15px 0 15px;'>
									<img src='".APP_PATH_IMAGES."exclamation_orange.png'><span style='font-size:14px;'>".RCView::tt("data_entry_532")."</span></div>";
			}
		...
		if (!isset($_REQUEST['updaterecs']) && isset($_REQUEST['submit']))
		{
			...
			$importData = $doODM ? file_get_contents($uploadedfile_name) : DataImport::csvToArray($uploadedfile_name, $_REQUEST['format'], $_REQUEST['delimiter']);
			...
			$result = Records::saveData($saveDataFormat, $importData, $overwriteBehavior, $dateFormat, 'flat',
										$user_rights['group_id'], true, true, false, false, true, array(), true, !$doODM, false, $forceAutoNumber);
					...
					print  "<div id='commit_import_div' class='darkgreen' style='max-width:850px;padding:20px;'>
								...
								<div id='uploadmain2'>
									...
									<div style='padding-top:10px;'>
										<button name='updaterecs' class='btn btn-xs btn-rcgreen fs14' onclick='if (importDataSubmit($require_change_reason)) document.form.submit();'>{$lang['data_import_tool_29']}</button>
		...
		elseif (isset($_REQUEST['updaterecs']))
		{
			...
			// Do test import to check for any errors/warnings
			$result = Records::saveData($saveDataFormat, $importData, $overwriteBehavior, $dateFormat, 'flat',
										$user_rights['group_id'], true, true, true, false, true, $change_reasons, false, !$doODM, false, $forceAutoNumber);
