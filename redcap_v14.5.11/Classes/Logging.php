...
class Logging
{
	...
	public static function renderLogRow($row, $html_output=true)
	{
        ...
        if ($row['description'] == 'Send alert' || strpos($row['description'], "PDF (with data)") !== false) {
            $this_record = $row['pk'];
    ...
    public static function getFilterHTML($getArr, $isApi = false) {
        ...
        $sql = "SELECT u.username, CONCAT(i.user_firstname, ' ', i.user_lastname) AS `name` 
                FROM redcap_user_rights u, redcap_user_information i 
                WHERE u.project_id = ? AND u.username != '' AND u.username = i.username";
        $q = db_query($sql, [$project_id]);
        while ($row = db_fetch_array($q)) {
            $this_user = strtolower(trim($row['username']));
            $this_name = trim($row['name']);
            $all_users[$this_user] = $this_name;
        }

        //Call log_event table for past users
        $logging_table = Logging::getLogEventTable($project_id);
        $sql = "SELECT DISTINCT user FROM $logging_table WHERE project_id = ? and ts >= ?";
        $q = db_query($sql, [$project_id, str_replace([" ","-",":"], [""], $Proj->project['creation_time'])]);
        while ($row = db_fetch_array($q)) {
            $this_user = trim(str_replace("SYSTEM\n", "", $row['user']));
            if (in_array($this_user, ["", "ADMIN"])) continue;
            $this_user_lc = strtolower($this_user);
            if (!isset($all_users[$this_user_lc])) {
                $userInfo = User::getUserInfo($this_user);
                $all_users[$this_user_lc] = $userInfo ? trim($userInfo['user_firstname']." ".$userInfo['user_lastname']) : "";
            }
        }
