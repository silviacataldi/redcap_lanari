...
class MultiLanguage
{
    ...
    public static function exitSurvey(Context $context = null) {
        ...
        // Language set in URL?
        if($context->lang_id == null && isset($_GET[MultiLanguage::LANG_GET_NAME])) {
            $context = Context::Builder($context)->lang_id($_GET[MultiLanguage::LANG_GET_NAME])->Build();
        }
    ...
    public static function translateASIAttributes(Context $context, &$asi_attrs) {
        ...
        foreach (["asi-email_sender_display","asi-email_subject","asi-email_content"] as $type) {
            $translation = $dd[$type][$asi_meta["form"]][$asi_id]["translation"];
    ...
    public static function parseIniFile($content) {
        ...
        foreach ($ui_meta as $key => $info) {
            if ($info["type"] == "string" && isset($ini[$key]) && $ini[$key] != $info["default"]) {
                $items[] = array(
                    "id" => $key,
                    "translation" => $ini[$key],
                );
    ...
    public static function getTranslationSettings(Context $context, $do_piping, $blocklist = []) {
        ...
        foreach($active_langs as $lang_id) {
            ...
            foreach ($ec->getEconsentSettings($context->project_id) as $attr) {
                ...
                $pdf_url = "";
                if (isinteger($this_lang_econsent['consent_form_pdf_doc_id'])) {
            ...
            if (isset($ref["surveys"][$context->instrument])) {
                foreach ($ref["surveys"][$context->instrument] as $survey_element => $ref_data) {
                    if (starts_with($survey_element, "survey-") && ($survey_settings_allowed == null || in_array($survey_element, $survey_settings_allowed))) {
                        ...
                        $survey_settings["ref"]["survey-translations"][$survey_element] = self::performPiping($context, $ref_data["reference"], false, $lang_id, $wrap);
                    }
                }
            }
            // User interface translations (use language string id as key where possible)
	...
	{
		$tryDraftMode = ($tryDraftMode && self::inDraftMode($project_id));
        // Serve from cache?
    ...
    public static function getProjectMetadata($project_id, $tryDraftMode=false) {
        ...
        if ($mycap_enabled) {
            ...
            foreach ($links as $link) {
                $mycap_settings["links"][$link["link_id"]] = [
                    ...
                    "mycap-link-link_name" => [
                        ...
                        "refHash" => self::getChangeTrackHash($contact["link_name"]),
                        "prompt" => ensureUTF(RCView::tt("multilang_723")), //= Translation of the link name:
                        "mode" => "text",
                    ],
                    "mycap-link-link_url" => [
                        "reference" => $link["link_url"],
                        "refHash" => self::getChangeTrackHash($contact["link_url"]),
            ...
            foreach ($tasks as $task_id => $task) {
                ...
                $task_items = [];
                // Task Title is available for every task and lives outside of the event context
                if (isset($task_items_template["task_title"])) {
                    $val = $task["task_title"];
                    $task_items["task_title"] = $task_items_template["task_title"];
                    $task_items["task_title"]["reference"] = $val;
                    $task_items["task_title"]["refHash"] = self::getChangeTrackHash($val);
                    unset($task_items_template["task_title"]);
                }
                ...
                foreach ($schedules as $task_event_id => $task_schedule) {
                    ...
                    foreach($task_items_template as $task_item_key => $task_item_base) {
                        ...
                        $val = isset($ext_config[$task_item_key_as_idx]) ? $ext_config[$task_item_key_as_idx] : $task_schedule[$task_item_key_as_idx];
    ...
    public static function getUIMetadata($is_system = false) {
        ...
        $metadata = array (
            ...
            "survey_1243" => [
                ...
                "prompt" => RCView::lang_i("multilang_335", [$lang["survey_1243"]]), //= The '{0}' button label:
            ],
            "survey_1244" => [
                "category" => "ui-captcha",
                "group" => "captcha",
            ...
            "data_entry_429" => [
                ...
                "type" => "string",
                "id" => "data_entry_429",
                "default" => $lang["data_entry_429"], // Type:
                "prompt" => RCView::tt("multilang_397"), //= The eConsent type title:
            ],
            "survey_1264" => [
                "category" => "ui-survey",
                "group" => "econsent",
            ...
            "survey_1339" => [
                ...
                "type" => "string",
                "id" => "survey_1339",
                "default" => $lang["survey_1339"], // Your email address will not be associated with or stored with your survey responses.
            ...
            "survey_504" => [
                ...
                "prompt" => RCView::lang_i("multilang_335", [$lang["survey_504"]]), //= The '{0}' button label:
            ],
            "data_entry_174" => [
                "category" => "ui-survey",
                "group" => "queue",
            ...
            "config_functions_61" => [
                ...
                "group" => "validation_messages",
                "type" => "string",
                "id" => "config_functions_61",
                "default" => $lang["config_functions_61"], // This field must be a 10 digit U.S. phone number (like 415 555 1212). Please re-enter it now.
    ...
    public static function getExportFile($project_id, $user, $payload) {
        ...
        $settings = $project_id == "SYSTEM" 
            ...
            : self::getProjectSettings($project_id);
        ...
        if ($request["items"] == null) {
            self::log("Export", "Language [{$lang_id}] was exported.", $user, $project_id);
        }
        else {
            self::log("Export", "Changed items of language [{$lang_id}] were exported.", $user, $project_id);
        }
        // Create export
        ...
        $time = time();
        $content = self::exportLanguage($project_id, $lang_id, $time, true, $include_prompt, $include_ref, $limit, $request["mode"], $request["items"]);
        // Add instructions
     ...
     * @return array 
     */
    private static function exportLanguage($project_id, $lang_id, $time, $full = false, $include_prompt = false, $include_ref = false, $limit = false, $mode = "all", $items = null) {
        ...
        if (!$limit || $limit["events"]) {
            ...
            foreach ($pmd["events"] as $event_id => $meta) {
                ...
                if (!self::isEmpty($meta["event-name"]["reference"])) {
                    ...
                    $sub_item["translation"] = $val;
                    if (
                        ($full || !self::isEmpty($val))
                        &&
                        (!$subset || isset($items["event-name"][$event_id]))
                       ) {
                        $item["name"] = $sub_item;
                    }
                }
                ...
                if (!self::isEmpty($meta["event-custom_event_label"]["reference"])) {
                    ...
                    if (
                        ($full || !self::isEmpty($val))
                        &&
                        (!$subset || isset($items["event-custom_event_label"][$event_id]))
                       ) {
            ...
            foreach ($pmd["fields"] as $fieldname => $meta) {
                ...
                if (!self::isEmpty($meta["field-header"]["reference"]) || !self::isEmpty($val)) {
                    ...
                    if (
                        ($full || !self::isEmpty($val))
                        &&
                        (!$subset || isset($items["field-header"][$fieldname]))
                       ) {
                ...
                if (!self::isEmpty($meta["field-label"]["reference"]) || !self::isEmpty($val)) {
                    ...
                    if (
                        ($full || !self::isEmpty($val))
                        &&
                        (!$subset || isset($items["field-label"][$fieldname]))
                       ) {
                        $item["label"] = $sub_item;
                    }
                }
                ...
                if (!self::isEmpty($meta["field-note"]["reference"]) || !self::isEmpty($val)) {
                    ...
                    if (
                        ($full || !self::isEmpty($val))
                        &&
                        (!$subset || isset($items["field-note"][$fieldname]))
                       ) {
                        $item["note"] = $sub_item;
                    }
                }
                ...
                if (!self::isEmpty($meta["field-video_url"]["reference"]) || !self::isEmpty($val)) {
                    ...
                    if (
                        ($full || !self::isEmpty($val))
                        &&
                        (!$subset || isset($items["field-video_url"][$fieldname]))
                       ) {
                ...
                if (isset($meta["field-enum"])) {
                    ...
                    foreach ($meta["field-enum"] as $choice => $choiceData) {
                        ...
                        if (
                            ($full || !self::isEmpty($val))
                            &&
                            (!$subset || isset($items["field-enum"][$fieldname][$choice]))
                           ) {
                ...
                if (isset($meta["field-actiontag"])) {
                    ...
                    foreach ($meta["field-actiontag"] as $index => $atData) {
                        ...
                        if (
                            ($full || !self::isEmpty($val))
                            &&
                            (!$subset || isset($items["field-actiontag"][$fieldname][$index]))
                           ) {
                            $at_items[] = $at_item;
                        }
                    }
            ...
            foreach ($pmd["matrixGroups"] as $matrixname => $meta) {
                ...
                if (!self::isEmpty($meta["matrix-header"]["reference"]) || !self::isEmpty($val)) {
                    ...
                    if (
                        ($full || !self::isEmpty($val))
                        &&
                        (!$subset || isset($items["matrix-header"][$matrixname]))
                       ) {
                        $item["header"] = $sub_item;
                    }
                }
                ...
                if (isset($meta["matrix-enum"])) {
                    ...
                    foreach ($meta["matrix-enum"] as $choice => $choiceData) {
                        ...
                        if (
                            ($full || !self::isEmpty($val))
                            &&
                            (!$subset || isset($items["matrix-enum"][$matrixname][$choice]))
                           ) {
                            $enum_items[] = $enum_item;
                        }
                    }
                    ...
                    if (starts_with($type, "survey-")) {
                        ...
                        if (!self::isEmpty($meta["reference"]) || !self::isEmpty($val)) {
                            ...
                            if (
                                ($full || !self::isEmpty($val))
                                &&
                                (!$subset || isset($items[$type][$form_name]))
                               ) {
                ...
                foreach ($asi as $type => $meta) {
                    if (starts_with($type, "asi-") && !self::isEmpty($meta["reference"])) {
                        ...
                        if (
                            ($full || !self::isEmpty($val))
                            &&
                            (!$subset || isset($items[$type][$asi_id]))
                           ) {
                        ...
                        if ($type == "alert-sendgrid_template_data") {
                            ...
                            foreach ($template_data as $key => $reference_value) {
                                ...
                                if (
                                    ($full || !self::isEmpty($val))
                                    &&
                                    (!$subset || isset($items[$type][$alert_id]))
                                   ) {
            ...
            foreach ($pmd["mdcs"] as $mdc => $meta) {
                if (!self::isEmpty($meta["reference"])) {
                    ...
                    if (
                        ($full || !self::isEmpty($val))
                        &&
                        (!$subset || isset($items["mdc-label"][$mdc]))
                       ) {
        ...
        if ((!$limit || $limit["mycap"]) && isset($pmd["myCapEnabled"])) {
            ...
            foreach ($mycap["mycap-baseline_task"] as $itype => $itype_meta) {
                if (!empty($itype_meta["reference"])) {
                    ...
                    if ($include_prompt) $task_items[$itype]["prompt"] = self::de_html($itype_meta["prompt"]);
                    if ($include_ref) $task_items[$itype]["default"] = $itype_meta["reference"];
                    $task_items[$itype]["translation"] = $dd[$itype][""][""]["translation"] ?? "";
                }
            }
            ...
            foreach ($mycap["taskToForm"] as $task_id => $form_name) {
                ...
                foreach ($pmd["myCap"]["orderedListOfTaskItems"] as $task_item_name => $_) {
                    ...
                    if (!empty($tasks[$task_item_name]["reference"])) {
                        ...
                        if ($include_prompt) $task_items[$dd_type]["prompt"] = self::de_html($tasks[$task_item_name]["prompt"]);
                        if ($include_ref) $task_items[$dd_type]["default"] = $tasks[$task_item_name]["reference"];
                        $task_items[$dd_type]["translation"] = $dd[$dd_type][$task_id][""]["translation"] ?? "";
                    }
                }
                // Task items specific to events
                foreach ($pmd["events"] as $event_id => $event) {
                    $event_id_task_id = "$event_id-$task_id";
                    if (!isset($tasks[$event_id_task_id])) continue;
                    foreach ($pmd["myCap"]["orderedListOfEventSpecificTaskItems"] as $task_item_name => $_) {
                        $dd_type = "task-$task_item_name";
                        if (!empty($tasks[$event_id_task_id][$task_item_name]["reference"])) {
                            $task_items[$event["uniqueEventName"]][$dd_type] = [
                                "hash" => $tasks[$event_id_task_id][$task_item_name]["refHash"],
                            ];
                            if ($include_prompt) $task_items[$event["uniqueEventName"]][$dd_type]["prompt"] = self::de_html($tasks[$event_id_task_id][$task_item_name]["prompt"]);
                            if ($include_ref) $task_items[$event["uniqueEventName"]][$dd_type]["default"] = $tasks[$event_id_task_id][$task_item_name]["reference"];
                            $task_items[$event["uniqueEventName"]][$dd_type]["translation"] = $dd[$dd_type][$event_id_task_id][""]["translation"] ?? "";
                        }
                    }
                }

                if (count($task_items) > 2) {
                    $export["myCapTranslations"][] = $task_items;
                }
            }
            #endregion
    ...
    public static function createSnapshot($project_id, $user) {
        ...
        try {
            ...
            foreach (array_keys($settings["langs"]) as $lang_id) {
                ...
                $lang_export = self::exportLanguage($project_id, $lang_id, $time);
    ...
    public static function getChoiceLabelTranslation(Context $context, $field_name, $code) {
        ...
        else if ($Proj->metadata[$field_name]["element_type"] == "truefalse") {
            return self::getDDTranslation($context, "field-enum", $field_name, $code);
        }
        // Determine enum type (matrix or field enum)
        if ($Proj->metadata[$field_name]["grid_name"] != null) {
            $enum_type = "matrix-enum";
            $enum_name = $Proj->metadata[$field_name]["grid_name"];
                ...
                foreach ($proj_meta['forms'][$formName]['myCapTaskItems'] as $event_task_id => $myCapTaskItem) {
                    if ($event_task_id != 'task_title') {
                        ...
                        if ($isActive) {
                            ...
                            $extendedConfigs = $proj_meta['forms'][$formName]['myCapTaskItems'][$event_task_id];
                            foreach ($extendedConfigs as $key => $config) {
                                if (starts_with($key, "extendedConfig_")) {
                                    ...
                                    if (isset($dd['task-'.$key][$event_task_id][""]['refHash']) && $dd['task-'.$key][$event_task_id][""]['refHash'] == $configRefHash) {
                                        $extendedConfigArr[$configKey] = $dd['task-'.$key][$event_task_id][""]["translation"];
                                    } else {
                                        $extendedConfigArr[$configKey] = $config['reference'];
                                    }
                                }
                                $tasks_details[$tsId][$eventId]['extendedConfigJson'] = json_encode($extendedConfigArr);
                            }
                        } else {
                            unset($tasks_details[$tsId][$eventId]['extendedConfigJson']);
            ...
            foreach ($tasks_details as $tsId => $tasks_detail) {
                ...
                if ($Proj->longitudinal) {
                    ...
                    foreach ($schedules as $eventId => $schedule) {
                        ...
                        if (isset($tasks_detail[$eventId]['extendedConfigJson'])) {
                            $task_arr['extendedConfigJson'] = $tasks_detail[$eventId]['extendedConfigJson'];
                        }
                        $result = $task_arr;
                        unset($task_arr['instructionStep'], $task_arr['completionStep'], $task_arr['extendedConfigJson']);
                        $result['identifier'] = $eventId;
                        $tasks['longitudinalSchedule'][] = $result;
                    }
                } else {
                    unset($tasks['instructionStep'], $tasks['completionStep'], $tasks['extendedConfigJson']);
                    ...
                    if (isset($tasks_detail[$Proj->firstEventId]['extendedConfigJson'])) {
                        $tasks['extendedConfigJson'] = $tasks_detail[$Proj->firstEventId]['extendedConfigJson'];
                    }
                }
                $ts_arr[] = $tasks;
            }
     ...
     * @return string
     */
    public static function getMyCapAppLanguageList() {
        $mcLangs = ['en-US' => '(American) English',
                    'it-IT' => 'Italian',
        ...
        foreach ($mcLangs as $langCode => $langText) {
            ...
            // Add entry to table
            $html .= '<tr valign="top"><td class="mt-5 boldish fs14 text-dangerrc">'.$langCode.'</td><td style="padding-left: 5px;">'.$langText.'</td></tr>';
            if ($i == 9 || $i == 18) {
                $html .= '</table></td>';
            }
