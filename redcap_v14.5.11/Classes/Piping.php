...
//                'survey-last-instance-sentorscheduled:'.$lang['global_277'] => array($lang['piping_79'], array("<code>[survey-duration-completed:prescreener]</code>", "93"), array("<code>[survey-duration-completed:prescreener:m]</code>", "12.7"), array("<code>[visit_1_arm_1][survey-duration-completed:prescreener:d][last-instance]</code>", "3.89"))
			...
			$lang['global_181']=>array(
				...
				'scatter-plot:'.$lang['global_284'].','.$lang['global_285'].','.$lang['global_286'].':'.$lang['global_280'] => array($lang['global_195']." ".$lang['global_289'], array("<code>[scatter-plot:height]</code>", $lang['global_190']), array("<code>[scatter-plot:height,weight]</code>", $lang['global_190']), array("<code>[scatter-plot:height,weight,gender]</code>", $lang['global_190'])),
				'line-chart:'.$lang['global_284'].','.$lang['global_285'].','.$lang['global_286'].':'.$lang['global_280'] => array($lang['global_196']." ".$lang['global_289'], array("<code>[line-chart:visit_date,weight]</code>", $lang['global_190']), array("<code>[line-chart:visit_date,weight,gender]</code>", $lang['global_190'])),
				'bar-chart:'.$lang['global_282'].','.$lang['global_286'].':'.$lang['global_280'] => array($lang['global_194']." ".$lang['global_289'], array("<code>[bar-chart:race]</code>", $lang['global_190']), array("<code>[bar-chart:race,gender]</code>", $lang['global_190'])),
			...
			$lang['global_198']=>array(
				...
				'_____:_____:bar-vertical' => array($lang['global_205'], array("<code>[bar-chart:race:bar-vertical]</code>", $lang['global_190']), array("<code>[bar-chart:race,gender:bar-vertical]</code>", $lang['global_190'])),
				'_____:_____:bar-stacked' => array($lang['global_206'], array("<code>[bar-chart:race,gender:bar-stacked]</code>", $lang['global_190']), array("<code>[bar-chart:race,gender:bar-vertical,bar-stacked]</code>", $lang['global_190'])),
				'_____:_____:no-export-link' => array($lang['global_209'], array("<code>[stats-table:age,race,gender:no-export-link]</code>", $lang['global_190']))
			),
                                           ...
                                           $mlm_target_lang=false, $isEmailContent=false, $isUsedInLogicCalcBranching=false)
	{
        ...
        if ($foundTags) 
		{
			...
			foreach ($matches['event_name'] as $key => $value) 
			{
                ...
                if (strpos($thisEventField, ':') !== false) list ($thisEventField, $nothing) = explode(":", $thisEventField, 2);
                if (strpos($matches['event_name'][$key], '-event-name') !== false && isset($Proj->metadata[$matches['command'][$key]])) {
                    $thisEventField =$matches['command'][$key];
                    $thisEvent = $matches['event_name'][$key];
                    $thisEventFieldForm = $Proj->metadata[$thisEventField]['form_name'];
                    $this_event_id = null;
...
//						}
						...
						if (isset($fields[0]) && $Proj->isCheckbox($fields[0])) {
							foreach (array_keys(parseEnum($Proj->metadata[$fields[0]]['element_enum'])) as $raw_coded_value) {
                        ...
                        if ($value == 'bar-chart' || $value == 'pie-chart' || $value == 'donut-chart') {
							// Build labels
							$labels = $Proj->isSqlField($fields[0]) ? parseEnum(getSqlFieldEnum($Proj->metadata[$fields[0]]['element_enum'])) : parseEnum($Proj->metadata[$fields[0]]['element_enum']);
							...
							if (isset($fields[1]) && ($Proj->isMultipleChoice($fields[1]) || $Proj->isSqlField($fields[1]))) {
								$categorize = true;
								$labelsGroups = $Proj->isSqlField($fields[1]) ? parseEnum(getSqlFieldEnum($Proj->metadata[$fields[1]]['element_enum'])) : parseEnum($Proj->metadata[$fields[1]]['element_enum']);
							...
							$xAxisLabel = $smartParams['barHorizontal'] ? "" : strip_tags($Proj->metadata[$fields[0]]['element_label']);
							$yAxisLabel = $smartParams['barHorizontal'] ? strip_tags($Proj->metadata[$fields[0]]['element_label']) : "";
							// Output chart
							if ($isPDFContent) {
                        ...
                        // SCATTER PLOT or LINE CHART
							...
							if (isset($fields[2]) && ($Proj->isMultipleChoice($fields[2]) || $Proj->isSqlField($fields[2]))) {
								$categorize = true;
								$labels = $Proj->isSqlField($fields[2]) ? parseEnum(getSqlFieldEnum($Proj->metadata[$fields[2]]['element_enum'])) : parseEnum($Proj->metadata[$fields[2]]['element_enum']);
							...
							if ($isPDFContent) {
								...
								if (($dashObject->isPublicDash() || $dashObject->isSurveyPage() || $dashObject->isSurveyQueuePage()) && !$canDisplayDataOnPublicDash) {
                                    ...
                                    $types = array(	($Proj->isMultipleChoice($fields[0]) ? "category" : $Proj->metadata[$fields[0]]['element_validation_type']),
													(isset($fields[1]) ? $Proj->metadata[$fields[1]]['element_validation_type'] : ""));
									...
									$return_value = Piping::renderSmartChart(self::getSmartChartId(), ($value == 'line-chart' ? 'line' : 'scatter'), $newdata, $labelsNums, "", 0, strip_tags(label_decode($Proj->metadata[$fields[0]]['element_label'])), (isset($fields[1]) ? strip_tags(label_decode($Proj->metadata[$fields[1]]['element_label'])) : ""), !$useRandomYvalue, array_values($labels), $types);
									// In case we end up caching a non-public dashboard's content, add some invisible HTML tags to denote that the data does not meet the
									// minimum data point requirements for displaying it as a public dashboard, in which case we'll pipe the dashboard content in real time for the public dashboard.
									if (!$dashObject->isPublicDash() && !$canDisplayDataOnPublicDash) {
										ProjectDashboards::$currentDashHasPrivacyProtection = true;
						...
						// ALL SMART FUNCTIONS (excluding record count)
							...
							foreach ($fields as $this_field) {
								if ($Proj->isCheckbox($this_field)) {
									foreach (array_keys(parseEnum($Proj->metadata[$this_field]['element_enum'])) as $raw_coded_value) {
							...
							// Pull field data from all records
								...
								$getDataParams = ['project_id' => $project_id, 'fields' => $fields, 'returnFieldsForFlatArrayData'=>array_merge($fields, $fieldsCheckbox), 'removeMissingDataCodes'=>true,
												  'records'=>$smartParams['filterRecords'], 'events'=>$smartParams['filterEvents'], 'groups'=>$smartParams['filterDags']];
								$data = Records::removeNonBlankValuesAndFlattenDataArray(Records::getData($getDataParams), false, false, $isSingleCheckbox);
							}
							// Can display data on public dashboard?
							$dashObject = new ProjectDashboards();
                    ...
                    case "new-instance" :
						...
						if ($fieldVar != "" && $matches['instance'][$key] != "" && $matches['event_name'][$key] != "") {
                            ...
                            if ($event_name == 'previous-event-name') {
                                ...
                                $event_id = $Proj->getFirstEventIdInArmByEventId($context_event_id, $Proj->metadata[$fieldVar]['form_name']);
                            } elseif ($event_name == 'last-event-name') {
                                $event_id = $Proj->getLastEventIdInArmByEventId($context_event_id, $Proj->metadata[$fieldVar]['form_name']);
...
//                        }
                            ...
                            if ($value == "survey-link" && $link != "") {
                                if (is_numeric($survey_id) && isset($Proj->surveys[$survey_id])) {
                                    $survey_title = ($matches['param2'][$key] != null) 
										...
										// value should be used. This is the only place in piping, where this is
										// implemented (others might be: instrument names, event names)
										: ($mlm_target_lang === false 
											? $Proj->surveys[$survey_id]['title'] 
					...
					case "survey-duration-completed" :
						...
						if ($value == "survey-date-completed" || $value == "survey-time-completed") {
							...
							if (!$returnRawValue && $surveyCompleted != self::missing_data_replacement) {
							...
							// Format the date/time to user preference if we're in a piping context (as opposed to calc/logic context, which would use raw value)
							if (!$returnRawValue && $surveyStarted != self::missing_data_replacement) {
				...
				if ($value == 'first-event-name' || $value == 'last-event-name' || $value == 'previous-event-name' || $value == 'next-event-name' || $value == 'event-name') {
					...
					if (isset($Proj->metadata[$this_field])) {
						...
						$matches['post-pipe'][$key] = is_numeric($this_event_id)
													...
													: ($replaceWithUnderlineIfMissing ? $wrapper.self::missing_data_replacement.$wrapper : $wrapper.$wrapper);
						$matches['pre-pipe'][$key] = "/" . preg_quote($matches[0][$key], '/') . "/";
						// Wrap the value in quotes or do SQL escape?
						if ($escapeSql) {
							$matches['post-pipe'][$key] = db_escape($matches['post-pipe'][$key]);
	...
	public static function replaceEmbedVariablesInLabel($label='', $project_id=null, $form=null, $replaceCurlyBracketsWithSquare=false, $replaceCurlyBracketWithUnderscore=false)
	{
		...
		foreach ($fields as $key=>$this_field) {
			...
			if ($replaceCurlyBracketWithUnderscore) {
				if ($Proj->metadata[$this_field]['form_name'] != $form && $form != 'ALL') continue;
				...
				} elseif ($Proj->metadata[$this_field]['element_type'] == "file" && $Proj->metadata[$this_field]['element_validation_type'] != "signature") {
				...
				$otherFormClass = ($Proj->metadata[$this_field]['form_name'] != $form && $form != 'ALL') ? ' embed-other-form' : '';
...
//		 print_array($repeating_instruments);
		...
		foreach ($fields as $key=>$this_field)
		{
            ...
            if ($this_instance == '') $this_instance = $instance; // If the instance attached to the field is blank, then we assume the instance of the current context
			...
			$string_replacement = 	// For text/notes fields, make sure we double-html-encode these + convert new lines
									...
									(($field_type == 'textarea' || $field_type == 'text')
										? filter_tags(str_replace(array("\r","\n"), array("",""),
                                            ...
                                            (
                                                (strpos($Proj->metadata[$this_field]['misc'] ?? "", "@RICHTEXT") !== false)
                                                ? $data_value
                                                : nl2br($data_value)
                                            )
                                          ))
