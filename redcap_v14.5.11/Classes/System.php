...
class System
{
	...
	const minimum_php_version_required = '7.3.0';
	// Set a list of the recommended versions of PHP
	public static $recommendedPhpVersions = array("8.0", "8.1", "8.2", "8.3");
	...
	public static $encryptedConfigSettings = array(
            ...
            "openid_connect_client_secret", "bioportal_api_token", "fieldbank_nih_cde_key",
	...
	{
        global $read_replica_hostname, $read_replica_username, $read_replica_password, $read_replica_db;
        return (isset($read_replica_hostname) && $read_replica_hostname != '' && isset($read_replica_username) && isset($read_replica_password) && isset($read_replica_db));
	}
	...
	public static function init()
	{
		...
		define('ALLOWED_TAGS', '<abbr><input><button><col><colgroup><strike><s><style><code><video><audio><source><caption><canvas><ol><ul><li><label><pre><p><a><br><center><font><b><i><u><h6><h5><h4><h3><h2><h1><hr><table><tbody><tr><th><td><thead><tfoot><img><span><div><em><strong><acronym><sub><sup><map><area>');
		// Set error handler
    ...
    if ($GLOBALS['twilio_enabled_global'] == '1' || ($GLOBALS['two_factor_auth_enabled'] == '1' && $GLOBALS['two_factor_auth_twilio_enabled'] == '1')) {
        TwilioRC::init();
    }
	...
	public static function setClickJackingControl()
	{
		...
		$launcherActive = FhirLauncher::inEhrLaunchContext();
		// If we are performing an EHR Launch for CDP, then ignore
        if ($launcherActive != false || basename($_SERVER['PHP_SELF']) == 'ehr.php') {
			return;
		}

        // If enabled (or if on Password Recovery page), then set header
		if ($clickjacking_prevention == '1' || basename($_SERVER['PHP_SELF']) == 'password_recovery.php') {
	...
	public static function getConfigVals()
	{
		...
		while ($row = db_fetch_assoc($q))
		{
			...
			if ($row['field_name'] == 'config_settings_key' && !isset($GLOBALS['config_settings_key'])) {
	...
	public static function setConfigVals()
	{
		...
		if ($GLOBALS['redcap_updates_password_encrypted'] == '0' && $GLOBALS['redcap_updates_password'] != '' && strpos($_SERVER['PHP_SELF'], "/ControlCenter/") !== false && basename($_SERVER['PHP_SELF']) != "install.php")
		{
	...
	public static function REDCapErrorHandler($code, $message, $file, $line)
	{
		...
		elseif ($isError)
		{
            ...
            self::addErrorToRCErrorLogTable("Error message: ".str_replace(["\t", "\r\n", "\r", "\n", "  ", "  "], " ", $message)."\nFile: $file\nLine: $line");
			...
			if (strpos($message, "Maximum execution time of") !== false)
			{
				...
				{
					\ExternalModules\ExternalModules::onShutdown();
	...
	public static function checkCsrfToken()
	{
		...
		if (!$exemptFromTokenCreate)
		{
			...
			$_SESSION['redcap_csrf_token'][NOW] = sha1($salt . NOW . $userid);
		}
		...
		if (!isset($_SESSION['redcap_csrf_token']) || $redcap_csrf_token == null || !in_array($redcap_csrf_token, $_SESSION['redcap_csrf_token']))
		{
			...
			if ($redcap_csrf_token != null && $redcap_csrf_token != "")
			{
				...
				for ($this_sec_ago = -10; $this_sec_ago <= $sec_ago; $this_sec_ago++)
				{
					...
					if ($redcap_csrf_token == sha1($salt . $this_ts . $userid))
					{
...
//                    print_array(db_fetch_assoc($q));
    ...
    public static function getForeignKeyByCol($table, $col, $refcol=null)
    {
        $sql = "select constraint_name, column_name, referenced_column_name from information_schema.KEY_COLUMN_USAGE 
				where CONSTRAINT_SCHEMA = DATABASE() and TABLE_NAME = '".db_escape($table)."' and referenced_column_name is not null";
        $q = db_query($sql);
        while ($row = db_fetch_assoc($q)) {
            $constraint_name = $row["constraint_name"] ?? ($row["CONSTRAINT_NAME"] ?? "");
            $column_name = $row["column_name"] ?? ($row["column_name"] ?? "");
            $referenced_column_name = $row["referenced_column_name"] ?? ($row["referenced_column_name"] ?? "");
            if ($constraint_name == '') continue;
            if ($col != null && $col == $column_name) return $constraint_name;
            if ($refcol != null && $refcol == $referenced_column_name) return $constraint_name;
        }
        return null;
    }
}
