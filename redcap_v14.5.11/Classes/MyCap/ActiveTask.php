...
class ActiveTask
{
    ...
    public static function insertDefaultTaskSetting($form_name, $form_label, $task_format, $extendedConfigAsString) {
        ...
        // Save task info
        $sql = "REPLACE INTO redcap_mycap_tasks (project_id, form_name, task_title, question_format, card_display)
			    VALUES (".PROJECT_ID.", '" . db_escape($form_name) . "', '" . db_escape($task_title) . "', '" . db_escape($task_format) . "', 
			            '" . db_escape($card_display) . "')";
        ...
        if (db_query($sql)) {
            ...
            if (!$Proj->longitudinal) {
                $sql_task_schedule = "REPLACE INTO redcap_mycap_tasks_schedules (task_id, event_id, schedule_relative_to, schedule_type, schedule_relative_offset, schedule_ends, extended_config_json)
			            VALUES (".$task_id.", ".$Proj->firstEventId.", '".Task::RELATIVETO_JOINDATE."', '".Task::TYPE_INFINITE."', '0', '".Task::ENDS_NEVER."', '" . db_escape($extendedConfigAsString) . "')";
    ...
    public static function getAllActiveTasks($third_party_platform = '')
    {
        ...
        $mtbTasks['Mobile Toolbox Tasks'][] = array(
