...
class Task
{
    ...
    public static function getMyCapTaskErrors($form = '') {
        ...
        if ($form == '') {
            ...
            if ($Proj->longitudinal) {
                ...
                if (!empty($events)) {
                    $errors[] = $lang['mycap_mobile_app_829']." <code>".implode(", ", $events)."</code>";
                }
    ...
    public static function getTaskSchedules($taskId = '', $flag = '', $projectId = null) {
        ...
        if (!$Proj->longitudinal && $flag == '') {
            $params[] = $Proj->firstEventId;
            $condition = "AND event_id = ?";
        }
        $sql = "SELECT * FROM redcap_mycap_tasks_schedules WHERE task_id = ? ".$condition." ORDER BY event_id";
    ...
    public static function getNonRepeatingFormsErrorsText($formsRepeating) {
        ...
        foreach ($formsRepeating as $form => $events) {
            if (!empty($events)) {
                $list .= '<li style="padding-top: 5px;">'.$lang['mycap_mobile_app_712'].' "'.$Proj->forms[$form]['menu'].'" '.$lang['mycap_mobile_app_828'].' <code>'.implode(", ", $events).'</code>';
            }
        }
        if ($list != '') {
    ...
    public static function csvTaskSchedulesDownload() {
        ...
        $result = [];
        $sql = "SELECT 
                    task_id, form_name, task_title, question_format, card_display, x_date_field, x_time_field, y_numeric_field
                FROM 
                    redcap_mycap_tasks 
            ...
            if (empty($taskErrors)) {
                ...
                if ($Proj->longitudinal) {
                    ...
                    if (!empty($schedules)) {
                        ...
                        foreach ($schedules as $eventId => $schedule) {
                            $i++;
                            $scheduleStr .= $Proj->eventInfo[$eventId]['name_ext'].' : '.Task::getDetailedSchedulesForEvent($schedule);
