...
class Promis
{
     ...
     * @param \MyCap\Study $study
     * @param \MyCap\App $app
     * @return string|null
     * @throws \Exception
     */
    public function setupIfNeeded($instrument, $eventId, $projectId)
    {
        ...
        if ($isPromisInstrument) {
            ...
            if (isset($myCapProj->tasks[$instrument]['task_id'])) {
                ...
                $sql = "SELECT extended_config_json FROM redcap_mycap_tasks_schedules WHERE task_id = $taskId AND event_id = $eventId AND form_name = '".db_escape($instrument)."' LIMIT 1";
                $q = db_query($sql);
                if (db_num_rows($q)) {
