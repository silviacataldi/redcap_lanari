...
$moduleMosioDisabled = (UserRights::isSuperUserNotImpersonator() || !$mosio_enabled_by_super_users_only) ? "" : "disabled";
...
$moduleDdpDisabled = ($DDP->isEnabledInSystem() && !UserRights::isSuperUserNotImpersonator()) ? "disabled" : "";
$moduleDdpFhirChecked = ($DDP->isEnabledInSystemFhir() && $DDP->isEnabledInProjectFhir()) ? "checked" : "";
$moduleDdpFhirDisabled = ($DDP->isEnabledInSystemFhir() && !UserRights::isSuperUserNotImpersonator()) ? "disabled" : "";
$displaySuperUserOnlySettings = (UserRights::isSuperUserNotImpersonator() && ($DDP->isEnabledInSystem() || $DDP->isEnabledInSystemFhir()
                                || ($twilio_enabled_global && $twilio_enabled_by_super_users_only) || ($mosio_enabled_global && $mosio_enabled_by_super_users_only)));
...
$checkList[$stepnum++] = array("header" => '<i class="fas fa-cubes fs14"></i> '.$lang['setup_95'], "status" => $moduleStatus, "name" => "modules",
	"text" =>   // If in production, give note regarding why options above are disabled
				...
				(!($DDP->isEnabledInSystem() && (UserRights::isSuperUserNotImpersonator() || (!UserRights::isSuperUserNotImpersonator() && $realtime_webservice_display_info_project_setup))) ? '' :
					RCView::div(array('style'=>$smProjectSetupBtnStyle.'margin-bottom:2px;color:'.($DDP->isEnabledInProject() ? 'green' : '#800000').';'),
                        ...
                        '<i class="ms-1 fas '.($DDP->isEnabledInProject() ? 'fa-check-circle' : 'fa-minus-circle').'" style="text-indent:0;"></i> ' .
				...
				(!($DDP->isEnabledInSystemFhir() && (UserRights::isSuperUserNotImpersonator() || (!UserRights::isSuperUserNotImpersonator() && $fhir_display_info_project_setup))) ? '' :
					RCView::div(array('style'=>$smProjectSetupBtnStyle.'margin-bottom:2px;color:'.($DDP->isEnabledInProjectFhir() ? 'green' : '#800000').';'),
                        ...
                        '<i class="ms-1 fas '.($DDP->isEnabledInProjectFhir() ? 'fa-check-circle' : 'fa-minus-circle').'" style="text-indent:0;"></i> ' .
				...
				(!($mosio_enabled_global && (UserRights::isSuperUserNotImpersonator() || $mosio_display_info_project_setup || !$mosio_enabled_by_super_users_only)) ? '' :
					RCView::div(array('style'=>$smProjectSetupBtnStyle.'margin-bottom:2px;color:'.($twilio_enabled && $mosio_api_key != '' ? 'green' : '#800000').';'),
						RCView::button(array('id'=>'enableMosioBtn','class'=>'btn btn-defaultrc btn-xs fs11','style'=>'','onclick'=>"dialogMosioEnable();",
							$disableBtn=>$disableBtn,$disableProdBtn=>$disableProdBtn,$moduleMosioDisabled=>$moduleMosioDisabled,$moduleMosioChecked=>$moduleMosioChecked),
