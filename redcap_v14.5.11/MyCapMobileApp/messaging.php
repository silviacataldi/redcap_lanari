...
elseif (isset($_POST['action']) && $_POST['action'] == "saveActionNeeded")
{
    ...
    if (db_query($sql)) {
        ...
        // error_log("===groupIds to postNotification ===");
        // error_log(print_array($groupID, true));
        ...
        // error_log("===codesList to postNotification ===");
        // error_log(print_array($codesList, true));
            ...
            if ($sendAnnouncement == true) {
                ...
                foreach ($codesList as $code) {
                    ...
                    if (strlen($details[$code]['push_notification_ids'])) {
                        $pushIds = json_decode($details[$code]['push_notification_ids']);
                        if (is_array($pushIds)) {
                            // error_log("===pushIds to postNotification ===");
                            // error_log(print_array($pushIds, true));
                            MyCapConfiguration::postNotification([
                                'deviceIds' => $pushIds,
                                'category' => 1,
                                'data' => [
