...
elseif (isset($_GET['__closewindow']))
{
    ...
    if (isset($_GET['pid'])) {
		require_once dirname(dirname(__FILE__)) . "/Config/init_project.php";
		MultiLanguage::exitSurvey(Context::Builder()
			->is_survey()
	...
	Survey::exitSurvey(RCView::tt("survey_1241")."<br>", true, false, false);
}
...
if (!isset($survey_context["project_id"])) {
    MultiLanguage::exitSurvey();
    Survey::exitSurvey(RCView::tt("survey_14"));
}
// Now set $_GET['pid'] before calling init_project
...
$nav_warning = isset($_COOKIE["redcap-survey-nav-alert"]) && $_COOKIE["redcap-survey-nav-alert"] == "1";
...
if (!$surveys_enabled || $survey_enabled < 1 || $date_deleted != '' || $status == 2 || $completed_time != '') {
	...
	MultiLanguage::exitSurvey(Context::Builder()
		...
		->instrument($_GET["page"])
		->Build());
	Survey::exitSurvey($offlineText, !$useCustomText);
}
...
if (!$public_survey && !Survey::checkSurveyTimeLimit($participant_id, $survey_time_limit_days, $survey_time_limit_hours, $survey_time_limit_minutes)) 
{
	...
	MultiLanguage::exitSurvey(Context::Builder()
		...
		->instrument($_GET["page"])
		->Build());
	Survey::exitSurvey(RCView::tt("survey_1105"));
}
...
if (!$public_survey && $_SERVER['REQUEST_METHOD'] == 'POST' && isset($_GET['__startover']) && isset($_POST['__response_hash__']))
{
    ...
    if ($randomization && Randomization::setupStatus() && Randomization::wasRecordRandomized($fetched, $project_id))
    {
        $criteriaFields = Randomization::getRandomizationFieldsParsed();
        // Remove randomization field from $eraseFields
        if ($criteriaFields[1] == $event_id && in_array($criteriaFields[0], $eraseFields)) {
            $randKey = array_search($criteriaFields[0], $eraseFields);
            if ($randKey !== false) unset($eraseFields[$randKey]);
            $randKey = array_search($thisField, $eraseFieldsLoggingKey);
            if ($randKey !== false) unset($eraseFieldsLogging[$randKey]);
        }
        // Remove strata fields from $eraseFields
        if (isset($criteriaFields[2])) {
            foreach ($criteriaFields[2] as $thisField=>$thisEvent) {
                if ($thisEvent == $event_id && in_array($thisField, $eraseFields)) {
                    $randKey = array_search($thisField, $eraseFields);
                    if ($randKey !== false) unset($eraseFields[$randKey]);
                    $randKey = array_search($thisField, $eraseFieldsLoggingKey);
                    if ($randKey !== false) unset($eraseFieldsLogging[$randKey]);
                }
		...
		if (Survey::surveyLoginFailedAttemptsEnabled())
		{
			...
			if ($failedLogins >= $survey_auth_fail_limit) {
				...
				MultiLanguage::exitSurvey(Context::Builder()
					...
					->instrument($_GET["page"])
					->Build());
				Survey::exitSurvey($surveyAuthFailedHtml);
			}
		}
	...
	if (isset($return_code))
	{
		...
		if (!$responseExists) {
			...
			MultiLanguage::exitSurvey(Context::Builder()
				...
				->instrument($_GET["page"])
				->Build());
            Survey::exitSurvey(RCView::tt("survey_111"), true, null, false);
		} else {
...
if (!$end_survey_redirect_next_survey && !$save_and_return && !$public_survey && !isset($_POST['submit-action']))
{
    ...
    if ($responseCompleted)
    {
        ...
        Survey::exitSurvey($full_acknowledgement_text, true, null, false);
    }
}
...
if ($_SERVER['REQUEST_METHOD'] == 'GET' && $save_and_return_code_bypass != '1')
{
	...
	if ($response_exists)
	{
		...
		elseif (!isset($_GET['__endsurvey']))
		{
			...
			{
                FormDisplayLogic::overrideSurveysStatuses($this_record, $event_id, $form_name, $_GET['instance']);
			...
			if (db_num_rows($q)) {
				...
				if ($completion_time_existing_record != "" && !$edit_completed_response) {
					...
					MultiLanguage::exitSurvey(Context::Builder()
						...
						->instrument($_GET["page"])
						->Build());
					Survey::exitSurvey(RCView::tt("survey_111"), true, null, isset($_POST['submit-action']));
				}
			}
		}
	}
		...
		if (isset($_POST['__response_id__']))
		{
			...
			if ($completion_time_existing_record != "" && !$edit_completed_response) {
				...
				MultiLanguage::exitSurvey(Context::Builder()
					...
					->instrument($_GET["page"])
					->Build());
				Survey::exitSurvey(RCView::tt("survey_111"));
			}
		}
				...
				<div style="<?php if ($save_and_return_code_bypass != '1') print "padding:5px 20px;margin-left:2em;text-indent:-2em;"; ?>">
						...
						<span id="provideEmail" style="<?=!$public_survey ? "display:none;" : ""?>">
							...
							<span style="font-size:10px;color:#800000;font-family:tahoma;">
								* <?=RCView::tt("survey_1339") // Your email address will not be associated with or stored with your survey responses. ?>
							</span>
						</span>
    ...
    || ($_SERVER['REQUEST_METHOD'] == 'GET' && isset($_GET['__endpublicsurvey']) && isset($_GET['__rh'])))
{
	...
	{
        FormDisplayLogic::overrideSurveysStatuses($fetched, $event_id, $form_name, $_GET['instance']);
		// Get the next survey url
	...
	else
	{
		...
		$this_acknowledgement = $surveyEndedViaStopAction ? $stop_action_acknowledgement : $acknowledgement;
		$mlm_acknowledgement_type = $surveyEndedViaStopAction ? "survey-stop_action_acknowledgement" : "survey-acknowledgement";
		...
		if ($confirmation_email_subject != '' && $confirmation_email_content != '' && !$deleteSurveyResponse)
		{
			...
			if (isset($emailsIdents[$_GET['id']]['email']) && $emailsIdents[$_GET['id']]['email'] == '') {
				...
				$full_acknowledgement_text .= RCView::div(array('style'=>'background-color:#EFF6E8;font-size:12px;margin:60px -11px 10px -11px;text-indent:-24px;padding:8px 12px 5px 36px;color:#333;border:1px solid #ccc;'),
                        ...
                        RCView::br() .
                        RCView::span(array('style'=>'color:#800000;font-size:10px;font-family:tahoma;'), "* " . RCView::tt("survey_1339"))
					);
			}
		}
		...
		if ($end_of_survey_pdf_download)
		{
			...
			$full_acknowledgement_text .= 	RCView::div(array('style'=>'background-color:#f6eeee;font-size:13px;margin:60px -11px 10px -11px;text-indent:-24px;padding:8px 12px 8px 36px;color:#333;border:1px solid #ccc;'),
												...
												RCView::button(array('class'=>'jqbuttonmed', 'style'=>'text-indent:0px;margin-left:20px;', 'onclick'=>"
													...
													RCView::span(array('style'=>'vertical-align:middle;color:#A00000;'), RCView::tt("design_121"))
												)
											);
		}
...
$saveBtn = RCView::button(array('name'=>'submit-btn-saverecord', 'tabindex'=>'0', 'class'=>'jqbutton nowrap','style'=>'color:#800000;min-width:140px;','onclick'=>'$(this).button("disable");dataEntrySubmit(this);return false;'), $saveBtnText);
// Repeat Survey? (if repeating instrument enabled)
$repeatSurveyBtn = '';
...
if ((($pdf_econsent_system_enabled && Econsent::econsentEnabledForSurvey($Proj->forms[$_GET['page']]['survey_id'])) || $question_by_section) && $_GET['__page__'] > 1 && !$hide_back_button) {
	...
	$saveBtnRow = RCView::td(array('colspan'=>'2','style'=>'padding:15px 0;'),
					RCView::div(array('class'=>'col-12 col-md-6 text-center float-start', 'style'=>'margin-bottom:5px;'),
						RCView::button(array('name'=>'submit-btn-saveprevpage', 'tabindex'=>'0', 'class'=>'jqbutton nowrap','style'=>'color:#800000;min-width:140px;','onclick'=>'$(this).button("disable");dataEntrySubmit(this);return false;'),
...
if ($public_survey && $_GET['__page__'] <= 2 && isset($_POST['submit-action']) && isset($_POST['__response_id__']) && isinteger($_POST['__response_id__']))
{
    ...
    // Redirect
    redirectAsPost($private_survey_link, $_POST);
}
...
// SURVEY QUEUE LINK (if not a public survey and only if record already exists)
$survey_queue_link = "";
if (!$survey_queue_hide && isset($_POST['__response_id__']) && ($_SERVER['REQUEST_METHOD'] == 'GET' || isset($return_code)) && Survey::surveyQueueEnabled() && Survey::getSurveyQueueForRecord($_GET['id'], true, $project_id)) {
...
END;
    ...
    if (!isset($_COOKIE["PHPSESSID"])) {
        ...
        ?>
        <script>setCookieMin('PHPSESSID', <?=json_encode($session_id)?>, <?=$this_autologout_timer?>);</script>
        <?php
    }
}
// Remove __PHPSESSID from the URL if present
if (isset($_GET["__PHPSESSID"])) {
	?>
	<script>modifyURL(removeParameterFromURL(window.location.href, '__PHPSESSID'));</script>
	<?php
}
