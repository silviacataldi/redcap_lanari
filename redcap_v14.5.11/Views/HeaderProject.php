...
$logoHtml = "<div id='menu-div'>
				<div class='menubox' style='padding:0px 10px 0px 7px;'>
					...
					<div class='hang'>
						<i class='far fa-list-alt' style='text-indent:0;position:relative;margin-left:-1px;margin-right:4px;'></i><a href='".APP_PATH_WEBROOT_PARENT."index.php?action=myprojects" . (($auth_meth_global == "none" && $auth_meth_global != $auth_meth_global && $auth_meth_global != "shibboleth") ? "&logout=1" : "") . "'>{$lang['bottom_03']}</a>
						" .
					...
					($user_messaging_enabled
						? "<div class='hang user-messaging-left-item'>
								<i class='fas fa-comment-alt' style='text-indent:0;top:1px;position:relative;color:#3E72A8 !important;margin-right:5px;'></i><a href='javascript:;'>{$lang['messaging_09']}</a>".Messenger::renderHeaderIcon('project-page')."
							</div>"
						: ""
					) .
                    "<div class='hang'>
					    <i class='fas fa-envelope' style='text-indent:0;top:1px;position:relative;color:#A00000;margin-right:5px;'></i><a target='_blank' href='$contactAdminBtnLink' style='color:#A00000;' class='btn-contact-admin'>$contactAdminBtnText</a>
				    </div>" .
...
if (!empty($user_rights))
{
    ...
    if ($mycap_enabled && $mycap_enabled_global && $user_rights['mycap_participants'])
    {
        ...
        if ($actionNeededCount > 0) {
            $mycapMsgBlock = '<span class="badgerc ms-2">'.$actionNeededCount.'</span>';
        }
        // Output menu link
	...
	## DATA STATUS GRID
	$dataEntry .= "<div class='hang' style='position:relative;'><i class=\"fas fa-th\" style='text-indent: 0;'></i>&nbsp;&nbsp;<a href='".APP_PATH_WEBROOT."DataEntry/record_status_dashboard.php?pid=$project_id'>".RCView::tt("global_91")."</a></div>";
	if ($status < 1) {
	...
	if ($status < 1) {
		$dataEntry .=  "<div class='menuboxsub' style='position:relative;'>- ".
						(($user_rights['record_create'] && (isset($user_rights['forms']) && ($user_rights['forms'][$Proj->firstForm] == '1' || $user_rights['forms'][$Proj->firstForm] == '3'))) ? RCView::tt("bottom_64") : RCView::tt("bottom_73"))."</div>";
	}
		...
		|| (isset($fetched) && PAGE == "DataEntry/index.php"))
	{
		...
		if ($longitudinal && isset($_GET['event_id']) && is_numeric($_GET['event_id']))
		{
			...
			$instanceNum = ($is_repeating_event && $_GET['instance'] > 1) ? 
...
if ($user_rights['design']) {
	$appsMenu .= "<div class='hang'>".RCView::img(array('src'=>'dashboard1.png', 'style'=>'margin-left:-3px;')) ."</i><a href='".APP_PATH_WEBROOT."index.php?pid=$project_id&route=ProjectDashController:index'>{$lang['global_182']}</a></div>";
}
...
if ($user_rights['design'] && MultiLanguage::showProjectMenuItem($project_id)) {
	$appsMenu .= "<div class=\"hang\"><i class=\"fas fa-globe\" style='margin-right:2px;'></i>&nbsp;<a href=\"".APP_PATH_WEBROOT."index.php?pid={$project_id}&route=MultiLanguageController:projectSetup\">".RCView::tt("multilang_01")."</a>";
	if (MultiLanguage::isAdminDisabled($project_id)) {
		$appsMenu .= RCView::tt("global_23", "span", [
			"class" => "badge badge-danger",
			"style" => "text-indent:0;margin-left:1em;text-transform:uppercase;"
		]); // Disabled
	}
	$appsMenu .= "</div>";
}
...
if ($user_rights['data_logging'] == '1' || $user_rights['email_logging'] == '1') {
    ...
    if ($user_rights['data_logging']) {
		$appsMenu .= "<i class=\"fas fa-receipt\" style='margin-left:2px;margin-right:2px;'></i>&nbsp;&nbsp;<a href=\"" . APP_PATH_WEBROOT . "Logging/index.php?pid=$project_id\">" . $lang['app_07'] . "</a>";
	}
	...
	if ($GLOBALS['email_logging_enable_global'] && $user_rights['email_logging'] == '1') {
		$appsMenu .= "<i class=\"fas fa-mail-bulk\"></i>
					<a href=\"" . APP_PATH_WEBROOT . "index.php?route=EmailLoggingController:index&pid=$project_id\">".($Proj->project['twilio_enabled'] ? $lang['email_users_96'] : $lang['email_users_53'])."</a>";
	}
	$appsMenu .= "</div>";
}
// Field Comment Log
...
if ($user_rights['file_repository']) {
	$appsMenu .= "<div class='hang'><i class=\"fas fa-folder-open\"></i>&nbsp;&nbsp;<a href=\"" . APP_PATH_WEBROOT . "index.php?pid=$project_id&route=FileRepositoryController:index\">{$lang['app_04']}</a></div>";
}
//User Rights
...
if ($user_rights['lock_record_customize'] > 0 || $user_rights['lock_record'] > 0) {
	...
	$appsMenu .= "<div class='hang'><i class=\"fas fa-lock\" style='margin-left:2px;margin-right:1px;'></i>&nbsp;&nbsp;<a href=\"" . APP_PATH_WEBROOT . $lockingMgmtPage . "?pid=$project_id\">".$lang[($GLOBALS['esignature_enabled_global'] ? 'locking_36' : 'system_config_761')]."</a></div>";
}
// Randomization
...
$helpMenu = "<div class='menubox' style='color:#444;'>
				...
				<div id='menuvids' style='display:none;line-height:1.2em;padding:2px 0 0 16px;'>
					<div class='menuvid'>
						&bull; <a onclick=\"popupvid('redcap_overview_brief03.mp4')\" style='font-size:11px;' href='javascript:;'>".$lang['bottom_58']."</a>
					</div>
					<div class='menuvid'>
						&bull; <a onclick=\"popupvid('full_project_build01.mp4')\" style='font-size:11px;' href='javascript:;'>".$lang['bottom_115']."</a>
					</div>
					<div class='menuvid'>
					...
					<div class='menuvid'>
						&bull; <a onclick=\"popupvid('applications_menu01.mp4')\" style='font-size:11px;' href='javascript:;'>".$lang['training_res_97']."</a>
					</div>
					<div class='menuvid'>
						&bull; <a onclick=\"popupvid('app_overview_01.mp4')\" style='font-size:11px;' href='javascript:;'>".$lang['global_118']."</a>
					</div>
					<div class='menuvid'>
						&bull; <a onclick=\"popupvid('mlm01.mp4')\" style='font-size:11px;' href='javascript:;'>".$lang['multilang_01']."</a>
					</div>
				</div>
...
function renderGlassBreaker() {
	global $isIE, $project_id;
	$systemEnabled = GlassBreaker::isSystemEnabled($project_id);
	if(!$systemEnabled) return;
	
	$blade = Renderer::getBlade(APP_PATH_DOCROOT.'Classes/BreakTheGlass/templates');
	$break_the_glass_enabled = GlassBreaker::isEnabled($project_id);
	$browser_supported = !$isIE || vIE() > 10;
	if(!$browser_supported) return;
	return $blade->run('index', compact('break_the_glass_enabled'));
}
...
function makeCDISPanel() {
	...
	$makeBody = function() use($project_id, $lang) {
		...
		$tokenManager = new FhirTokenManager($fhirSystem, UI_ID, $project_id);
		$fhirUser = new FhirUser(UI_ID, $project_id);
		$mappedEhrUser = $fhirUser->getMappedEhrUser();
		$accessToken = $tokenManager->getToken();
		ob_start();
		?>
		<!-- <script type="module">
			import CDIS from '<?= APP_PATH_JS ?>cdis.js';

			document.addEventListener('DOMContentLoaded', (e) => {
				const cdis = new CDIS()

				const disconnectEhr = document.querySelector('[data-DisconnectEhr]')
				disconnectEhr.style.cursor = 'pointer'
				disconnectEhr.addEventListener('click', (e) => {
					const ehrUser = disconnectEhr.getAttribute('data-DisconnectEhr')
					cdis.disconnectEhrUser(ehrUser)
				})
			})
		</script> -->
		<div class="menubox">

			<!-- <div style="border-bottom: solid 1px #cacaca" class="d-flex">
				<div>
					<small>
						<i class="fas fa-id-card text-success"></i> EHR user: <strong><?= $mappedEhrUser ?></strong>
					</small>
				</div>
				<div class="ml-auto">
					<div class="dropdown">

						<i class="fas fa-bars" id="dropdownMenu2" data-toggle="dropdown"></i>
			...
			<span class="d-block">
				...
				<a href="<?= APP_PATH_WEBROOT.'ehr.php?standalone_launch=1&ehr_id='.$fhirSystem->getEhrId() ?>"><?= $lang['control_center_4893'] ?></a>
			</span>

			<?php if($fhirUser->can_use_mapping_helper) : ?>
			<span class="d-block">
				<i class="fas fa-code-branch"></i>
				<a href="<?= FhirMappingHelper::getLink($project_id) ?>"><?= $lang['control_center_4894'] ?></a>
			</span>
			<?php endif; ?>
			...
			<div class="access-token-wrapper mt-1" data-accessToken="<?= $accessToken ?>" >
				<span class="d-block small text-muted">
				<?php if($accessToken) : ?>
					<i class="fas fa-check-circle text-success"></i>
					<span><?= $lang['control_center_4895'] ?></span>
				<?php else : ?>
					<i class="fas fa-times-circle text-danger"></i>
					<span><?= $lang['control_center_4896'] ?></span>
				<?php endif; ?>
				</span>
			</div>
			...
			<div class="auto-login-wrapper">
				...
				<?php if($mappedEhrUser) : ?>
					...
					<span title="automatic log-in enabled during a launch from EHR with the user '<?= $mappedEhrUser ?>'"><?= $lang['control_center_4897'] ?></span>
				<?php else : ?>
					<i class="fas fa-times-circle text-danger"></i>
					<span title="no EHR user is mapped to your REDCap user"><?= $lang['control_center_4898'] ?></span>
				<?php endif; ?>
				</span>
			</div>
...
if($ehrLaunchContext && ($fhirSession instanceOf SessionDTO) && ($patientData = @$fhirSession->patientData))
{
	...
	$fhirNavbar = $renderer->run('partials.navbar', [
		'session' => $fhirSession,
...
if (defined("PROJECT_ID"))
{
	...
	if (FhirEhr::isFhirEnabledInProject($project_id)
		...
		if ($fhir_standalone_authentication_flow == FhirEhr::AUTHENTICATION_FLOW_STANDARD) {
			...
			$fhirSystem = FhirSystem::fromProjectId($project_id);
			$tokenManager = new FhirTokenManager($fhirSystem, $userid_fhir, $project_id);
...
$myCapSettings = MyCap::getMetaDataProjectSettings();
if (!empty($myCapSettings['data']) && isset($enabledModules['mycap']))
{
