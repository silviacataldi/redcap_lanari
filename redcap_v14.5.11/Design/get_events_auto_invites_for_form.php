...
foreach ($chooseEventRows as $this_event_id=>$attr)
{
	if ($attr['active'] == '1') {
		...
		$img = RCView::img(array('src'=>'tick_small_circle.png','style'=>'position:relative;top:-1px;'));
		$color = 'green';
		$btnTxt = $lang['design_169'];
	} elseif ($attr['active'] == '0') {
		// Add check icon with green text
		$img = RCView::img(array('src'=>'bullet_delete.png'));
		$color = '#800000';
		$btnTxt = $lang['design_169'];
	} else {
    ...
    // Set button and text
    $btn1 = RCView::button(array('class'=>'btn btn-xs btn-defaultrc fs11','style'=>"margin-right:5px;color:$color;",
