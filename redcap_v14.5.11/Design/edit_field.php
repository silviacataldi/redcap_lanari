...
if ($status > 0 && $draft_mode != '1') exit("<script type='text/javascript'>window.parent.window.alert(window.parent.window.woops);</script>");
...
if (!$super_user) {
    ...
    if ($_POST['field_type'] == "calc" && trim($_POST['element_enum']) != '') {
        $logic = LogicParser::removeComments($_POST['element_enum']);
    } // CALCTEXT
    elseif ($_POST['field_type'] == "text" && Calculate::isCalcTextField($_POST['field_annotation'])) {
        ...
        $logic = Calculate::buildCalcTextEquation($_POST, LogicParser::removeComments($_POST['misc'])); // Pass last param as override in case piping has resulted in a static value
    } // CALCDATE
    elseif ($_POST['field_type'] == "text" && Calculate::isCalcDateField($_POST['field_annotation'])) {
        $_POST['element_type'] = "text";
        $_POST['misc'] = $_POST['field_annotation'];
        $_POST['element_validation_type'] = $_POST['val_type'];
        $logic = Calculate::buildCalcDateEquation($_POST, false, LogicParser::removeComments($_POST['misc'])); // Pass last param as override in case piping has resulted in a static value
    }
}
