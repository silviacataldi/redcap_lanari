...
if ($_SERVER['REQUEST_METHOD'] == 'POST')
{
	...
	foreach ($_POST as $field=>$value) {
		...
		if ($value == "" && ($field == "ehr_id" || in_array($field, Project::$overwritableGlobalVars))) {
	        $value = "NULL";
        }
...
if (isset($_GET['project']) && isinteger($_GET['project']))
{
	...
	<tr>
		...
		<td class="cc_data">
			<select class="x-form-text x-form-field" style="" name="two_factor_exempt_project" onchange="
	...
	<tr>
		...
		<td class="cc_data">
			<select class="x-form-text x-form-field" style="" name="two_factor_force_project" onchange="
                if ($(':input[name=two_factor_exempt_project]').val() == '1' && $(':input[name=two_factor_force_project]').val() == '1') {
                    simpleDialog('<?php echo js_escape($lang['system_config_687']) ?>');
                    $(this).val('0');
                }
