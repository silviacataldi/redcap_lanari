...
if (isset($_GET['plottime'])) {
	...
	// User logins
		...
		$base_count = 0;
		$sql = "SELECT x.Date, sum(x.Count) as Count FROM (
					(select date(ts) as Date, count(1) as Count from redcap_log_view_old where ts > '$date_limit' and event = 'LOGIN_SUCCESS' group by Date)
					UNION
					(select date(ts) as Date, count(1) as Count from redcap_log_view where ts > '$date_limit' and event = 'LOGIN_SUCCESS' group by Date)
				) x group by Date";
		$isCumulative = false;
	// Projects Created
	...
	// Concurrent users within 30min blocks
		...
		$base_count = 0;
		$sql = "select x.Date, sum(x.Count) from (
					(SELECT left(FROM_UNIXTIME(floor(UNIX_TIMESTAMP(ts)/1800)*1800),16) as Date, count(distinct(session_id)) as Count
					from redcap_log_view where ts > '$date_limit'
					group by floor(UNIX_TIMESTAMP(ts)/1800))
					union
					(SELECT left(FROM_UNIXTIME(floor(UNIX_TIMESTAMP(ts)/1800)*1800),16) as Date, count(distinct(session_id)) as Count
					from redcap_log_view_old where ts > '$date_limit'
					group by floor(UNIX_TIMESTAMP(ts)/1800))
				) x 
				group by x.Date
				order by x.Date limit 1,".($day_span*48);
		$isDateFormat = false;
		$isCumulative = false;
...
if (isset($_GET['ddp1']) && isset($_GET['fhir']))
{
	...
	$total_ddp_records_imported = array();
	$total_ddp_projects_adjudicated = array();
	$sql = "select * from redcap_projects d, redcap_ehr_import_counts m 
			where d.project_id = m.project_id and m.type = 'CDP' and d.project_id in (" . prep_implode($ddp_project_ids) . ")";
	...
	while ($row = db_fetch_assoc($q)) {
		...
		$total_ddp_values_adjudicated += (int)$row['counts_Patient']+(int)$row['counts_Observation']+(int)$row['counts_Condition']
                                        +(int)$row['counts_Medication']+(int)$row['counts_AllergyIntolerance']
                                        +(int)$row['counts_Encounter']+(int)$row['counts_Immunization']+(int)$row['counts_AdverseEvent'];
	}
...
if (isset($_GET['data_mart']))
{
	...
	$q = db_query($sql, [], null, MYSQLI_USE_RESULT);
	while ($row = db_fetch_assoc($q)) {
		$total_data_mart_values_imported += (int)$row['counts_Patient']+(int)$row['counts_Observation']+(int)$row['counts_Condition']
											+(int)$row['counts_Medication']+(int)$row['counts_AllergyIntolerance']
											+(int)$row['counts_Encounter']+(int)$row['counts_Immunization']+(int)$row['counts_AdverseEvent'];
	}
	// Return values
