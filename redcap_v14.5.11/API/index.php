...
if (isset($_GET['type']) && $_GET['type'] == "module") {
	...
	if (isset($_GET['type']) && $_GET['type'] == "module" && isset($_GET['prefix']) && $_GET['prefix'] == "mycap") {
        if (isset($_POST['stu_code']) && $_POST['stu_code'] != '') {
            ...
            if ($projectId != '') {
                $myCapProj = new Project($projectId);
                if ($myCapProj->project['mycap_enabled'] == 1) {
...
if ($_POST["content"] == "externalModule" && $module_api_present) {
...
if ($_SERVER['REQUEST_METHOD'] == 'GET' && in_array($_GET['content'], array('mycap', 'tableau'))) {
...
elseif ($post['content'] == 'record' && in_array($post['action'], array('delete', 'rename'))) {
        $action = $post['action'];
}
else {
...
if ($tokenRequired && strlen($post['token']) != 64) {
    ...
    elseif (($action == "delete" || $action == "list" || $action == "createFolder") && ($post['content'] == "fileRepository" || $post['content'] == "user") && $post['api_import'] != 1) {
        die(RestUtility::sendResponse(403, "You do not have API Import/Update privileges"));
    }
