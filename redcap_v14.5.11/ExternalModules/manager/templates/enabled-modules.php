...
<div style="display:flex;" class="mt-2">
	<?=ExternalModules::tt("em_manage_131")?> <button class="btn btn-xs btn-rcred btn-rcred-light ms-2" onclick="actionTagExplainPopup(1);return false;" style="line-height: 14px;padding:1px 3px;font-size:11px;">@ Action Tags</button>
</div>
<script>
	...
	if (empty($versionsByPrefix)) {
		...
		foreach ($versionsByPrefix as $prefix => $version) {
			...
			if(empty($config)){
				...
				$config['name'] = ' ' . $prefix; // Prefix with a space to force sorting to the top
				$config['description'] = ExternalModules::tt("em_manage_132");
