...
class Framework
{
	...
	const SYSTEM_SUB_SETTINGS = 14;
	const INCLUDE_BY_DEFAULT_IN_PROJECTS_WITH_MODULE_ENABLED_FRAMEWORK_VERSION = 15;
	...
	function isREDCapPage($path): bool{
     ...
     */
    function dataDictionaryCSVToMetadataArray($csvFilePath, $delimiter = ",")
    {
    ...
    **/
    ...
    public function getDAG($recordId){
        ...
        $result = $this->query("select value from ".$this->getDataTable($pid)." where project_id = ? and event_id = ? and record = ? and field_name = ? limit 1", [$pid, $eventId, $recordId, '__GROUPID__']);
        ...
        if ($type == 'checkbox' || $type == 'dropdown' || $type == 'radio') {
            ...
            foreach ($choices as $choice) {
                ...
                if ($value != "") {
                    ...
                    } else if (strval($value) === $option_value) {
                        $label = $this->filterChoiceLabelFromChoice($choice);
                    }
    ...
    public function getAjaxSettings() {
        ...
        if ($user === \System::SURVEY_RESPONDENT_USERID) $user = null;
        ...
        if (self::isSurveyPage()) {
            ...
            if (!empty($survey_hash)) {
                ...
                if (isset($_GET["__dashboard"])) {
                     ...
                     */
                    $ajax_endpoint .= "&__dashboard=" . $_GET["__dashboard"];
                }
                else if (isset($_GET["__report"])) {
                    /**
                     * @psalm-taint-escape html
                     * @psalm-taint-escape has_quotes
                     */
                    $ajax_endpoint .= "&__report=" . $_GET["__report"];
                }
            }
        }
        else {
    ...
    function getDataTable($pid = null){
        $pid = $this->requireProjectId($pid);
    ...
    function getSurveyLinkNewInstance($this_form, $record, $pid = null){
        $pid = $this->requireProjectId($pid);
