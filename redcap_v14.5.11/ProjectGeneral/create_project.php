...
if ($isTemplate) {
    ...
    $sql = "select mycap_enabled from redcap_projects where project_id = ".$_POST['copyof'];
    $q = db_query($sql);
    $templateMyCapEnabled = (db_result($q, 0) == '1');
}
...
$sql = "insert into redcap_projects (project_name, scheduling, repeatforms, purpose, purpose_other, app_title, creation_time, created_by,
		...
		project_grant_number, surveys_enabled, mycap_enabled, auto_inc_set, randomization, template_id, data_resolution_enabled, project_note, datamart_enabled, log_event_table, data_table) values
		...
		".checkNull(trim($_POST['project_note'])).", ".($ehrDataMartProject ? '1' : '0').", '$log_event_table', '$data_table')";
$q = db_query($sql);
...
if (isset($_POST['copyof']) && is_numeric($_POST['copyof']))
{
        ...
        while ($row = db_fetch_assoc($q)) {
            $sql = "INSERT INTO redcap_mycap_tasks (project_id, form_name, enabled_for_mycap, task_title, question_format,
                    ...
                    VALUES
                        ...
                        " . checkNull($row['card_display']) . ", " . checkNull($row['x_date_field']) . "," . checkNull($row['x_time_field']) . "," . checkNull($row['y_numeric_field']) . ")";
            db_query($sql);
            $this_task_id = db_insert_id();

            // COPY ANY MYCAP TASKS SCHEDULES
            $sql2 = "SELECT * FROM redcap_mycap_tasks_schedules WHERE task_id = '".$row['task_id']."'";
            $q2 = db_query($sql2);
            while ($row2 = db_fetch_assoc($q2)) {
                $sql_schedule = "INSERT INTO redcap_mycap_tasks_schedules (task_id, event_id, allow_retro_completion, allow_save_complete_later, include_instruction_step, include_completion_step, instruction_step_title,
                        ...
                        " . checkNull($row2['schedule_end_after_days']) . ", " . checkNull($row2['schedule_end_date']) . ", " . checkNull($row2['extended_config_json']) . ")";
	...
	if ((!$isTemplate && isset($_POST['copy_records']) && $_POST['copy_records'] == "on") || ($isTemplate && $templateCopyRecords))
	{
		...
		{
			$sql = "INSERT INTO ".\Records::getDataTable($new_project_id)." (project_id, event_id, record, field_name, `value`, instance)
					select '$new_project_id', '$new_event_id', record, field_name, `value`, instance
					from ".\Records::getDataTable($copyof_project_id)." where project_id = $copyof_project_id and event_id = $old_event_id";
			db_query($sql);
		}
		...
		if (count($groupid_array) > 0) {
			...
			foreach ($groupid_array as $old_id=>$new_id) {
				$sql = "update ".\Records::getDataTable($new_project_id)." set value = '".db_escape($new_id)."' where project_id = $new_project_id
    ...
    if ($isTemplate || (isset($_POST['copy_formdisplaylogic']) && $_POST['copy_formdisplaylogic'] == "on"))
    {
        ...
        $sql = "SELECT hide_filled_forms, hide_disabled_forms, form_activation_survey_autocontinue FROM redcap_projects WHERE project_id = " . $copyof_project_id;
        $q = db_query($sql);
        $hide_filled_forms = db_result($q, 0, "hide_filled_forms");
        $hide_disabled_forms = db_result($q, 0, "hide_disabled_forms");
        $form_activation_survey_autocontinue = db_result($q, 0, "form_activation_survey_autocontinue");

        // Update hide_filled_forms value of new project
        $sql = "UPDATE redcap_projects SET hide_filled_forms = '$hide_filled_forms', hide_disabled_forms = '$hide_disabled_forms', form_activation_survey_autocontinue = '$form_activation_survey_autocontinue' where project_id = $new_project_id";
        db_query($sql);
    ...
    if ($templateMyCapEnabled || (isset($_POST['copy_mycap_mobile_app_content']) && $_POST['copy_mycap_mobile_app_content'] == "on"))
    {
        ...
        $sql = "INSERT INTO redcap_mycap_projects (code, hmac_key, project_id, name, converted_to_flutter) 
        ...
        if ($isTemplate) {
            $sqlConvertToFlutter = "UPDATE redcap_mycap_projects SET converted_to_flutter = '1' WHERE project_id = '".$new_project_id."'";
 ...
 */
	...
	if ($odmEdocId	|| $odmUpload || $ehrDataMartProject) {
		...
		$hasMyCapData = $odm_response['hasMyCapData'] ?? false;
...
if (isset($_POST['copyof']) && is_numeric($_POST['copyof'])) {
    ...
    $sql = "INSERT INTO redcap_mycap_projects (code, hmac_key, project_id, name) 
            VALUES ('".$myCap->generateUniqueCode()."',
                    '".$myCap->generateHmacKey()."',
                    '".$new_project_id."',
                    '".db_escape($_POST['app_title'])."')";
    ...
    foreach ($myCapTasks as $taskId => $attr) {
        if ($attr['question_format'] == Task::PROMIS) {
            $sql = "UPDATE redcap_mycap_tasks SET question_format = '".Task::QUESTIONNAIRE."', card_display = '".Task::TYPE_PERCENTCOMPLETE."'
                    WHERE project_id = '".$new_project_id."' AND task_id = '".$taskId."'";
            db_query($sql);
            $sql = "UPDATE redcap_mycap_tasks_schedules SET extended_config_json = NULL WHERE task_id = '".$taskId."'";
            db_query($sql);
        }
    }
