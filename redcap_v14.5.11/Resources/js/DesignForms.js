$(function(){
	...
	if (surveys_enabled > 0) {
		...
		$('.modsurvstg').mouseenter(function() {
...
function setFormMenuDescription(form_name,askChangeSurveyTitle, askChangeTaskTitle) {
	if (askChangeSurveyTitle == null) askChangeSurveyTitle = 0;
	if (askChangeTaskTitle == null) askChangeTaskTitle = 0;
	...
	$.post(app_path_webroot+'Design/set_form_name.php?pid='+pid, { page: form_name, action: 'set_menu_name', menu_description: this_value },
		function(data) {
			...
			var mycap_task_title = formArray[3];
			...
			if (askChangeSurveyTitle && survey_title != new_form_menu) {
				simpleDialog(langSetSurveyTitleAsForm1+' "<b>'+new_form_menu+'</b>"'+langQuestionMark+" "
					+langSetSurveyTitleAsForm2+' "'+survey_title+'"'+langPeriod, langSetSurveyTitleAsForm3,null,null,"checkMyCapTaskTitle('"+new_form+"', '"+askChangeTaskTitle+"', '"+mycap_task_title+"', '"+new_form_menu+"');",langNo,"setSurveyTitleAsFormLabel('"+new_form+"', '"+askChangeTaskTitle+"', '"+mycap_task_title+"', '"+new_form_menu+"');",langSetSurveyTitleAsForm4);
				return;
			} else if (mycap_enabled) {
				checkMyCapTaskTitle(new_form, askChangeTaskTitle, mycap_task_title, new_form_menu);
				return;
			}
...
function setSurveyTitleAsFormLabel(form, askChangeTaskTitle, mycap_task_title, new_form_menu) {
	$.post(app_path_webroot+'Design/set_survey_title_as_form_name.php?pid='+pid,{ form: form },function(data){
		if (data == '0') {
			alert(woops);
		} else {
			simpleDialog(langSetSurveyTitleAsForm5+' "<b>'+data+'</b>"'+langPeriod,langSetSurveyTitleAsForm6,null,null,"checkMyCapTaskTitle('"+form+"', '"+askChangeTaskTitle+"', '"+mycap_task_title+"', '"+new_form_menu+"');");
		}
	});
}

/**
 * Function to check MyCap task title
 */
function checkMyCapTaskTitle(new_form, askChangeTaskTitle, mycap_task_title, new_form_menu) {
	if (askChangeTaskTitle == 1 && mycap_task_title != new_form_menu) {
		simpleDialog(langSetTaskTitleAsForm1+' "<b>'+new_form_menu+'</b>"'+langQuestionMark+" "
			+langSetTaskTitleAsForm2+' "'+mycap_task_title+'"'+langPeriod, langSetTaskTitleAsForm3,null,null,"window.location.reload();",langNo,"setMyCapTaskTitleAsFormLabel('"+new_form+"');",langSetTaskTitleAsForm4);
		return;
	}
	if (status == 0) window.location.reload();
}
...
function deleteForm(form_to_delete, baseline_date_field_form) {
	...
	if (form_to_delete == baseline_date_field_form) {
...
function saveSurveyQueueSetupPopup() {
	...
	$.post(app_path_webroot+'Surveys/survey_queue_setup.php?pid='+pid, json_ob,function(data){
		if (data=='[]') alert(woops);
			...
			const $btn = $('#btnGroupDrop1');
			if (json_data.survey_queue_enabled == '1') {
				$btn.removeClass('dropdown-toggle');
				if ($btn.find('img[data-surveyqueue-enabled]').length === 0) {
					const $img = $('<img data-surveyqueue-enabled>');
					$img.css('position', 'relative').css('margin-left', '2px').attr('src', app_path_images + 'tick_small_circle.png').appendTo($btn);
				}
			} else {
				$btn.addClass('dropdown-toggle').find('[data-surveyqueue-enabled]').remove();
			}
		}
	});
}
...
function toggleSurveyQueueExport() {
	$.get(
		...
		function(data) {
			let sqsExportDisabled = jQuery.parseJSON(data).survey_queue_export_disabled;
			const obj = document.getElementById('SQS-container_dropdown_export');
			if (sqsExportDisabled) {
				...
				obj.setAttribute('onclick', 'SurveyQueueSetup.export()');
			}
		}
	);
}
...
function displayFormDisplayLogicPopup() {
	...
	$.post(app_path_webroot+'Design/form_display_logic_setup.php?pid='+pid,{action: 'view'},function(data){
		...
		if (data=='[]') alert(woops);
			...
			var $repeater = $('.repeater').repeater({
				show: function () {
					...
					$(".repeater-divs").each(function() {
						logicValidate($('#control-condition-'+counter), false, 1);
						$(this).find(".condition-number").html(counter);
						$(this).find("textarea").attr('id','control-condition-'+counter);
					...
					if (isFRSLPopupOpened && $("#FormDisplayLogicSetupDialog div.repeater-divs").length > 1) {
						$("#FormDisplayLogicSetupDialog div.repeater-divs:last").effect('highlight', {}, 1000);
					}
					setTimeout("fitDialog($('#FormDisplayLogicSetupDialog'));", 200);
				},
				hide: function (deleteControlElement) {
					var control_id = $(this).find("[id^='control_id']").val();
			...
			if (json_data.stored_data.enable_survey_support == '1') {
				$("#enable_survey_support").prop('checked', true);
			} else {
				$("#enable_survey_support").prop('checked', false);
			}
			$('#hide_disabled_forms').prop('checked', json_data.stored_data.hide_disabled_forms == '1');
			var controls = json_data.stored_data.controls;
...
function validationConditionSetupForm() {
	// Make sure all visible fields have a value
	var form_errors = logic_errors = 0;
	var errors = "";
...
function saveFormDisplayLogicSettings(ignorePotentialDuplicates = false) {
	...
	json_ob.enable_survey_support = $("#enable_survey_support").is(':checked') ? 1 : 0;
