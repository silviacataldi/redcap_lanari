 ...
 */
 ML.init = function(data) {
    ...
    $(function() {
        ...
        // @ts-ignore
        initialHash = objectHash(config.settings)
        setDirty(false)
...
function renderLanguagesTable() {
    ...
    for (const langKey of langKeys) {
        ...
        $row.find('[data-mlm-action="translate-ui"]').addClass((lang.syslang == '' ? '' : 'mlm-syslang ') + (lang.subscribed ? 'mlm-subscribed ' : '') + (uiOverridesAllowed ? 'mlm-can-override' : ''));
...
function setupAddEditLanguageModal() {
    addEditLanguageModal = (() => {
        ...
        /** Copies key, display, sort values from imported language */
...
var exportModal = {
    ...
    download: function() {
        ...
        expItems.mycap       = isProject && !isSingleFormExport && exportModal._$.find('input[name="export-include-mycap"]').is(":checked")
...
function updateFormsTable(lang) {
    var $formsTab = $('#mlm-forms')
    $formsTab.hide()
    var $tbody = $('#mlm-forms-rows')
    // Remove all rows
    $tbody.children().remove()
    // Create rows
    for (const formName of Object.keys(config.projMeta.forms)) {
        ...
        if (currentLanguage == config.settings.refLang) {
            ...
            if (config.projMeta.forms[formName].isPROMIS) {
                $row.find('[data-mlm-action="translate-fields"]').html('<i class="fas fa-lock text-danger"></i> ' + $lang.multilang_04)
            }
            else {
                $row.find('[data-mlm-action="translate-fields"]').html('<i class="far fa-eye-slash"></i> ' + $lang.multilang_04)
            }
            $row.find('[data-mlm-action="translate-survey"]').html('<i class="far fa-eye-slash"></i> ' + $lang.multilang_04)
            // Set ASI language source
            $row.find('[data-mlm-type="asi-source"]').attr('data-mlm-name', formName).val(config.settings.asiSources[formName])
        }
        else {
            if (config.projMeta.forms[formName].isPROMIS) {
                // Change field translation icon to indicated that PROMIS instruments cannot be translated
                $row.find('[data-mlm-action="translate-fields"]').html('<i class="fas fa-lock text-danger"></i> ' + $lang.multilang_83)
            }
        }
        if (form.isSurvey) {
...
function updateEventTaskTranslationStatus(container) {
    ...
    $container.find('[data-mlm-indicator="event-task-translated"]').removeClass('badge-light').addClass($container.find('[data-mlm-indicator].badge-danger').length == 0 ? 'badge-success' : 'badge-danger');
}
...
function updateDdTranslationStatus(name) {
    ...
    const $items = $('[data-mlm-field="' + name + '"], [data-mlm-matrix="' + name + '"]')
    if ($items.length < 1) {
...
const sitModal = {
    ...
    _pasteIntoRTE: function(text) {
        // @ts-ignore
        tinyMCE.editors["mlm-sit-rte"].execCommand('mceInsertContent', false, text, true)
    }
}
...
function set_dd(lang, type, name, index, val, hash = null, update = true) {
    ...
    if (update && !['survey-active','form-active'].includes(type)) {
        if (type.startsWith('survey-')) {
            ...
            updateDdTranslationStatus('task-' + config.projMeta.myCap.taskToForm[name]);
        }
...
function renderAlertsExclusionTable() {
    ...
    for (const alertId of getSortAlertIds()) {
        ...
        $row.find('[data-mlm-type="alert-excluded"]').prop('checked', config.settings.excludedAlerts[alertId] == true).attr('data-mlm-name', alertId)
        $row.find('[data-mlm-type="alert-source"]').attr('data-mlm-name', alertId).val(config.settings.alertSources[alertId])
        $tbody.append($row)
    }
...
/**
 * Checks the state and sets the dirty bit.
 */
function checkDirty() {
    // @ts-ignore
    const hash = objectHash(config.settings);
    if (hash == initialHash) {
        setDirty(false);
    }
    else {
...
function updateSourceDataChangedReport(showDialog = true) {
    ...
    for (const langKey of Object.keys(config.settings.langs)) {
        ...
        if (lang.hasOwnProperty('ui')) {
            for (const uiId of Object.keys(lang.ui)) {
        ...
        if (lang.hasOwnProperty('dd')) {
            for (const type of Object.keys(lang.dd)) {
                for (const name of Object.keys(lang.dd[type])) {
                    for (const index of Object.keys(lang.dd[type][name])) {
...
function getSingleFormExportItems(exportForm) {
    ...
    const items = {}
    // Fields
    for (const fieldName of Object.keys(config.projMeta.fields)) {
        const field = config.projMeta.fields[fieldName]
        if (field.formName == exportForm) {
            for (const type of Object.keys(field)) {
                switch (type) {
                    case 'field-header':
                    case 'field-label':
                    case 'field-note':
    ...
    // Matrix groups
    for (const matrixName of Object.keys(config.projMeta.matrixGroups)) {
                ...
                switch (type) {
                    ...
                    case 'matrix-enum':
                        if (matrix.hasOwnProperty(type) && typeof matrix[type] == 'object') {
                            for (const index of Object.keys(matrix[type])) {
    ...
    if (config.projMeta.hasOwnProperty('asis') && config.projMeta.asis != null) {
        for (const asiId of Object.keys(config.projMeta.asis)) {
            const asi = config.projMeta.asis[asiId]
            if (asi.form == exportForm) {
                for (const type of Object.keys(asi)) {
...
function showChangedHashItems(forceShow = false) {
    ...
    for (const langKey of Object.keys(itemsChangedSinceTranslated)) {
        ...
        $tbody.append($tr)
        // Items
        for (const type of Object.keys(itemsChangedSinceTranslated[langKey])) {
            for (const name of Object.keys(itemsChangedSinceTranslated[langKey][type])) {
                for (const index of Object.keys(itemsChangedSinceTranslated[langKey][type][name])) {
