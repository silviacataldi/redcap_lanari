<template>
    <div v-if="patient" class="border rounded p-2">
        <span class="fw-bold fs-3">{{
            `${patient?.['name-given']} ${patient?.['name-family']}`
        }}</span>
        <small class="d-block text-muted fst-italic total-entries">
            Total entries:
            <AnimatedCounter :value="overallTotal" v-slot="{ animating, text }">
        ...
        <div class="d-flex align-items-center gap-2">
            <small class="text-muted fst-italic">
                <span class="fw-bold">FHIR ID</span>: {{ patient?.fhir_id }}
            </small>
            <button
                class="btn btn-sm btn-outline-primary"
                @click="onCopyClicked(patient?.fhir_id)"
                :disabled="!patient?.fhir_id"
            >
        ...
        <div v-if="patient?.deceasedBoolean">
            <span class="badge bg-warning">
                ...
                <span>
                    {{ moment(patient?.deceasedDateTime).format('YYYY-MM-DD') }}
                </span>
            </span>
        </div>
