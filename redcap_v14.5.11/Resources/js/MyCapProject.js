    ...
    $('.schedule_type_sel').click(function() {
        ...
        if ($(this).is(':checked')){
            ...
            if (selectionVal == infiniteType || selectionVal == repeatingType) {
                ...
                $(this).closest('table').find("#typeSelection").html(label);
            } else {
...
/**
 * Function that displays setup event display format popup
 */
function displayEventDisplayFormatPopup() {
    showProgress(1,0);
    // Id of dialog
    var dlgid = 'settings_dialog';
    // Get content via ajax
    $.post(app_path_webroot+'MyCapMobileApp/custom_setup.php?pid='+pid,{action: 'view'},function(data){
        showProgress(0,0);
        if (data == "0") {
            alert(woops);
            return;
        ...
        $('#'+dlgid).dialog({ title: json_data.title, bgiframe: true, modal: true, width: 800, open:function(){ fitDialog(this); }, close:function(){ $(this).dialog('destroy'); },
            buttons: [{ text: "Cancel", click: function(){ $(this).dialog('close'); } },
                { text: "Save", click: function(){ saveCustomSettings(); }
                }]
        });
        $('#'+dlgid).parent().find('div.ui-dialog-buttonpane button:eq(1)').css({'font-weight':'bold','color':'#222'});
        // Init buttons
...
function saveCustomSettings() {
    var json_ob = $('form#customSettingsForm').serializeObject();
    json_ob.action = 'save';
    // Save via ajax
    $.post(app_path_webroot+'MyCapMobileApp/custom_setup.php?pid='+pid, json_ob, function(data){
        if (data=='[]') alert(woops);
        else {
            var json_data = jQuery.parseJSON(data);
            if ($('#settings_dialog').hasClass('ui-dialog-content')) $('#settings_dialog').dialog('destroy');
            simpleDialog(json_data.content, json_data.title, null, 500, "showProgress(1);window.location.reload();");
            setTimeout("showProgress(1);window.location.reload();", 2000);
        }
    });
