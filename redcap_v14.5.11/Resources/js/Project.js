...
function logicValidate(ob, longitudinal, forceMetadataTable) {
    ...
    var mssg = '<span class="logicValidatorOkay"><img src="'+app_path_images+'tick_small.png">'+window.lang.design_718+'<span>'+window.lang.global_172+'</span></span>';
    var err_mssg = "<span class='logicValidatorOkay'><img style='position:relative;top:-1px;margin-right:4px;' src='"+app_path_images+"cross_small2.png'>"+window.lang.global_171+'<span>'+window.lang.global_172+'</span></span>';
    // timeout to let new text value to enter field;
    // just to back of queue to process
    setTimeout(function() {
...
function specialFunctionsExplanation() {
    ...
    $.get(app_path_webroot+'DataEntry/special_functions_explanation.php'+(isProjectPage ? "?pid="+pid : ""),{},function(data){
        var json_data = JSON.parse(data);
        initDialog('special_functions_explain_popup');
                ...
                'Randomize': function() { // Randomize - ttfy
                    ...
                    $.post(app_path_webroot+'Randomization/randomize_record.php?pid='+pid+'&instance='+getParameterByName('instance'), { event_id: event_id, redcap_data_access_group: $('#random_form select[name="redcap_data_access_group"]').val(), existing_record: document.form.hidden_edit_flag.value, action: 'randomize', record: record, fields: critFlds.join(','), field_values: critFldVals.join(',') }, function(data){
                        ...
                        if (success) {
                            ...
                            var recordName = $('#randomizeDialog #record').val();
...
function toggleCurrentUsersCSVDownloadDropdown() {
    var initialOverflowState = {
        user_list: '',
        mDiv: ''
    };
    var user_list = document.getElementById('user_list');
    var mDiv = user_list.querySelector('.mDiv');
    if (initialOverflowState.user_list === '') {
        initialOverflowState.user_list = user_list.style.overflow;
        initialOverflowState.mDiv = mDiv.style.overflow;
    }
    if (user_list.classList.contains('flexigrid')) {
        mDiv.style.overflow = 'visible';
        user_list.style.overflow = 'visible';
    }
    // toggle dropdown menu
    var dropdownMenu = document.getElementById('currentUsersCSVDropdownMenu');
    dropdownMenu.style.display = (dropdownMenu.style.display === 'none') ? 'block' : 'none';
    // reset initial overflow state
    if (dropdownMenu.style.display === 'none') {
        mDiv.style.overflow = initialOverflowState.mDiv;
        user_list.style.overflow = initialOverflowState.user_list;
    }
    function handleClickOutside(event) {
        let dropdown = document.getElementById('currentUsersCSVDropdown');
        if (dropdownMenu.style.display === 'block' && !dropdown.contains(event.target)) {
            dropdownMenu.style.display = 'none';
            mDiv.style.overflow = initialOverflowState.mDiv;
            user_list.style.overflow = initialOverflowState.user_list;
            // remove event listener after dropdown is closed
...
function openLogicEditor(opener, fullscreen, onUpdateFn)
{
    ...
    dlgDiv.dialog({ bgiframe: true, modal: true, width: dialogWidth, title: window.lang.global_170, buttons:
        ...
        [{text: window.lang.global_53, click: function() {
            try{ $('#'+opener.prop('id')+'_Ok').hide(); }catch(e){ }
...
function loadInstancesTable(ob, record, event_id, form) {
    ...
    $.post(app_path_webroot+'index.php?pid='+pid+'&route=DataEntryController:renderInstancesTable', {record:record, event_id:event_id, form:form, display_close_icon:1 },function(data){
