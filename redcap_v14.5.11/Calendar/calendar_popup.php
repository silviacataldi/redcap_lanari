...
if (isset($_GET['cal_id']) && is_numeric($_GET['cal_id']) && empty($_POST))
{
	...
	if (db_num_rows($q) > 0) {
		...
		print  "	</td>
				...
				<tr valign='top'>
					...
					<td style='padding-top:5px;' id='td_notes'>
						...
						\">".RCView::escape($row['notes'])."</textarea>
						<div>
		...
		if (isset($row['record']) && $row['record'] != "" && $row['event_id'] != "")
		{
			...
			// Render form list
            $formlist = renderPanel(RCView::tt('global_61'), "<div style='padding:2px 0;'>$formString</div>");
			print "<div id='data_entry_forms' style='font-size:14px;width:240px;border-top:1px solid #d0d0d0;'>$formlist</div>";
		}
