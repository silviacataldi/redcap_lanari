...
if (MyCap\MyCap::checkIfValidTaskOfProject($form, $_GET['task_id'])) {
    ...
    if ($_SERVER['REQUEST_METHOD'] == "POST") {
        ...
        $sql = "UPDATE redcap_mycap_tasks SET task_title = '" . db_escape($task_title) . "', question_format = '" . db_escape($question_format) . "',
                    ...
                    enabled_for_mycap = '" . $enabled_for_mycap . "'
                    WHERE task_id = ?";
        ...
        if (db_query($sql, [$task_id])) {
            ...
            foreach ($_POST as $key => $value) {
                if (strpos($key, 'extendedConfig_') !== false) {
                    ...
                    if (strpos($key, 'extendedConfig_numberOfDisks') !== false) $value = (int)$value;

                    list ($param, $event_id) = explode("-", $key_parts[1]);
                    $extendedConfigData[$event_id][$param] = $value;
                }
            }
            foreach ($_POST['tsevent'] as $eventId) {
                ...
                if ($active == 1) {
                    ...
                    if ($isPromis) {
                        ...
                        $taskObj->setupIfNeeded($form, $eventId, PROJECT_ID);
                    } else if ($is_active_task == 1 && $is_mtb_task == 0) {
                        $taskObj = MyCap\ActiveTask::getActiveTaskObj($question_format);
                        $taskObj->buildExtendedConfig($extendedConfigData[$eventId]);
                    }
                ...
                if ($active == 1 && $num_rows == 0) { // if checked and not exists in DB then insert new entry
                    ...
                    if ($skipUpdate == false) {
                        ...
                        $sql = "UPDATE redcap_mycap_tasks_schedules SET 
                                    ...
                                    schedule_end_after_days = " . checkNull($schedule_end_after_days) .", extended_config_json = '" . db_escape($extendedConfigAsString) . "'";
                        $sql .= ", schedule_end_date = " . checkNull($schedule_end_date);
                        $sql .= " WHERE task_id = '".$task_id."' AND event_id = '".$eventId."'";
                    }
                } elseif ($active == 0) {
                    $sql = "DELETE FROM redcap_mycap_tasks_schedules WHERE event_id = " . $eventId . " AND task_id = " . $task_id;
                }
                if ($skipUpdate == false) {
                    db_query($sql);
                }
            }
            $sql = "DELETE FROM redcap_mycap_tasks_schedules WHERE event_id NOT IN (" . implode(", ", $_POST['tsevent']) . ") AND task_id = " . $task_id;
            db_query($sql);
...
if (!$Proj->longitudinal && !$Proj->isRepeatingFormAnyEvent($form)) {
