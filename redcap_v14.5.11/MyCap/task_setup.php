    ...
    case 'activetask':
        ...
        if (($is_active_task == 1 && !$isPromis) || $isBatteryInstrument) {
            if (is_null($extended_config_json) || empty($extended_config_json)) {
                $taskObj = ActiveTask::getActiveTaskObj($question_format);
                $taskObj->buildExtendedConfig($extendedConfigData);
                $extended_config_json = isset($taskObj) ? ActiveTask::extendedConfigAsString($taskObj) : null;
            }
            ...
            if (!empty($configs)) {
            ?>
                <tr data-event="event_<?php echo $eventId;?>" id="tr-activetasks-<?php echo $eventId;?>">
                    <td>
                        <table width="100%">
                            <tr>
                                <td colspan="3">
                                    <div class="header" style="padding:7px 10px 5px;margin:-5px -8px 10px; color: #800000;"><i class="fas fa-cog"></i>
                                        <span id="activeTaskHeading">
                                            <?php echo ($isBatteryInstrument) ? 'Health Measure Battery' : ActiveTask::toString($question_format)." ".RCView::tt('multilang_72').RCView::tt('colon'); ?>
                                        </span>
                                        <?php echo Task::getCopyToDropdownHTML($form, $eventId, 'activetasks'); ?>
                                    </div>
                                </td>
                            </tr>
                            <tr>
            ...
            <td>
                <table width="100%">
                    ...
                    <tr class="mycap_setting_row" id="endTaskFields" style="display: <?php echo $displayEndTaskFields;?>;" >
                        <td valign="top" style="font-weight:bold;padding-bottom:15px;">
                            <?php echo RCView::tt('mycap_mobile_app_825'); ?> <span id="typeSelection"><?php echo ltrim($schedule_type, "."); ?></span> <?php echo RCView::tt('mycap_mobile_app_826'); ?>
                        </td>
                        <td valign="top" style="padding-left:15px;padding-bottom:15px;">
                            <div>
